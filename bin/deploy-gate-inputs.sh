...
bin/deploy.sh	read	A6b	required-nonempty	gate_a6b_bash_floor	f0839cc511ef	refuses: phase B re-execs the DEPLOYED release's own copy to run the window, so a release without one (or with an empty one) is checked out with the app down and then has nothing to run. Present, its BASH_FLOOR line is the floor this host's bash is held to; a release carrying no such line predates card#9616 and PASSES, saying so. A line that IS present but is not `<major>.<minor>` is refused rather than read as far as it parses (card#9984): `4.x.5` was being enforced as the floor 4.0.5, which is not the floor that release declared. A1 holds THIS copy's own declaration to the same `bash_floor_is_version`.
...
server/package-lock.json	read	A12	required-nonempty	gate_a12_asset_lockfile	9a4ab0610879	refuses: npm ci needs it and package.json floats, so the prod asset build would not be reproducible. Its CONTENT is read too (card#9616): the lockfileVersion is the npm floor, and an EMPTY file is refused as empty — a distinct refusal from a lockfileVersion the gate cannot map, because the remediation differs.
bin/supervision.sh	read	A13	required-nonempty	gate_a13_target_plan	2e52651e950b	refuses: the window installs the deployed release's crontab block from it
