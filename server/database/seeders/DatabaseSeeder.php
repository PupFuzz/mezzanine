...
class DatabaseSeeder extends Seeder
{
     ...
     * `password` (`Database\Factories\UserFactory::definition()`). `database/factories/` is in
     * composer's PRODUCTION autoload section, not `autoload-dev`, so `php artisan db:seed` and
     * `php artisan migrate --seed` both work on a deployed host — and either one would have put an
     * account with a publicly known password behind the login page of a public deployment
     * (`docs/PLAN.md` D-03).
     *
