...
slots touch — the shipped default — a seat with interns reached into its neighbour's. Read from the scene's
...
- **Discriminating control:** the shipped default map — re-authored to the furniture box, which is row 14's
  obligation before this gate runs — with the four `fx-snapshot-4` seats passes every clause, so the gate is
  known to be able to say *nothing overlaps* and *no line*; and the cap leg on that map passes only once that
  re-authoring has landed — on the sprite-sized objects it ships today the cap desk cannot lie inside its rect,
  *(a)* fails on it, and *(f)*'s undersized line names every slot of the room, which is the measurement that
  says the re-authoring is owed rather than optional.
