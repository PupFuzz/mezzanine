...
**The three coordination rows belong to no seat either, and what they carry is stated here as a
FORWARD CONTRACT — because nothing on this floor writes one yet, and an implementer who reaches that
point should not have to guess it twice.** Doc-owner ruling, card#7341 step 6.
...
`cause` column above — which names four causing messages, none of them a `coord.round`, and reads a
seat object's `state_version` on a held row — answers nothing for them. On their rows `seat_id` is
...
⛔ **No consumer on this floor writes these rows today, and that is why
[AT-D3-1](#at-d3-1-no-animation-without-its-event)'s causing-message set is still FOUR.** The client
protocol applies neither `coord.thread` nor `coord.round` — `coord/coord-model.js` is the
coordination surface's own module, reached from its own page — so there is no path by which a row
above could be written, and widening that test's set now would add an arm no fixture can exercise,
which is a decoration rather than a gate. **The set gains `coord.round` and `coord.thread` in the
same change as the first consumer that applies them, together with the discriminating test that
fires one** — step 7 builds the line those rows are drawn on
([§ 14](#14-open-questions-for-the-review-loop) item 24), and the triggers themselves are already
built and are not to be re-minted: `threadAnimations()` and `roundAnimations()` decide them from the
rendered objects (card#8300), and Appendix B row 6's set declares each row's class and both of its
[§ 6.4](#64-reduced-motion-is-a-first-class-rendering-not-a-degradation) forms so that step reads
them off the closed set rather than answering twice.
