...
```
 ...
 8. the stream ends or errors          -> if its last message was feed.close, its reason says
                                         what to do (§ 9 F3, F7); otherwise as step 7
...
[§ 10.3](#103-the-floor-map) is where the default lives, whether it exists yet, and what holds `S` to it. Every value below is re-derived by
...
`tilewidth`, `tileheight` — and it always has one to read, because [Appendix B](#appendix-b--what-an-implementer-builds-from-this) step 7 lands the
default before step 11 lands the plan.
...
polling mode holds no subscription, sees no `building.layout`, and draws the old building until it
...
1. ⛔ **RESOLVE, OR RENDER UNRESOLVED — never a guessed desk.** Every name on both objects is a
   **protocol agent name**, and D2 publishes no mapping from one to a `seat_id`: *"That an agent name
   identifies the same thing a `seat_id` does is UNVERIFIED"*, and *"equality between an agent name
   and a `seat_id` is a coincidence this plane cannot check"*
   ([D2 § 8.3.3](FLEET-STATE.md#833-the-coordination-objects)). card#7957's ruling *(2)* is therefore
   the render rule, and it is correct on day one and stays correct after the join lands: a name that
   does not resolve to a desk **draws no line** — never to a guessed desk, never to nothing — **is
   reported as unresolved**, and **does not suppress the rest of the object**, so a round with one
   resolved and one unresolved participant renders what it can. ⚠ **No join exists today**, on any
   wire or in any config this repository owns, so today **every** name renders unresolved and **no
   line is drawn on any floor** — the thread renders as its beads, its label and its unresolved
   participants. That is the honest render of what the wire carries, not a degradation of one;
   [§ 14](#14-open-questions-for-the-review-loop) item 4 carries the join, and the renderer does not
...
fired by a `coord.round` whose address carries `all`. ⚠ **All three are unreachable today** for
clause 1's reason — a line needs two resolved endpoints and nothing resolves — and that is a property
of the join rather than of the renderer, so none of them is stubbed, softened or given a fallback
endpoint. **A19 and A20 both fire on one broadcast post and that is not one fact drawn twice**: the
...
|---|---|---|
| worst-case delta at the cap of 8 | **6,169 B** | D2, measured by serializing its published worst-case block |
| per-message bound | **8,192 B** (8 KiB) | D2 |
| spare | **2,023 B** | 8,192 − 6,169 |
| each further subagent element | **263 B** | D2, measured: the element at 262 B plus its comma |
| further elements that fit | **7** | ⌊2,023 ÷ 263⌋ = 7 |
| the cap could reach | **15** | 8 + 7, at a worst-case delta of **8,010 B** (6,169 + 7 × 263) |
| 16 breaches | **8,273 B** | 8,010 + 263, which is **81 B over** the 8,192 B bound |
...
3. **The 2,023 B of spare is worth more unspent, and card#8075 is what that means in practice.**
...
| F3 | **The server ended the stream on its own decision** — `feed.close` with `stalled` (the client stopped draining, [D2 § 8.5](FLEET-STATE.md#85-gaps-reconnect-and-why-state_version-is-not-seq)), `session` (the 15 s re-check **reached the store and the store said the session or its MFA is gone**, [D2 § 9](FLEET-STATE.md#9-read-side-authentication) — a re-check that could not REACH the store is D2's third outcome, *unverifiable*, and closes nothing: F21), or `unavailable` (the store was unreachable at connect, so the stream never got a cursor — the client treats it as an ordinary drop and reconnects on the 10 s cadence) | the message, followed by the stream's end ([D2 § 8.3](FLEET-STATE.md#83-the-websocket-delta-feed)) — SSE has no close frame, so the reason is the last **envelope** the client received before `error` fired, and D2 pins `eventStream()`'s `endStreamWith` to `null` so that nothing is appended after it. A stream that ends with **no** `feed.close` ended for a reason the server did not choose, and is F1's silence arriving early | `stalled`: **reconnecting** in the status strip, the floor keeps rendering with growing ages, and the client's event log records the reason; `session`: F6's render, immediately | `stalled`: re-run [§ 2.2](#22-connect-snapshot-deltas) from step 1 on the 10 s cadence; `session`: re-authenticate first — no reconnect attempt until then | blanking the floor while reconnecting; and, on `session`, reconnect-looping against a server that has said it will refuse |
| F4 | **Snapshot `503 fleet_unavailable`** | the status code and body ([D2 § 2.2](FLEET-STATE.md#22-fail-posture-per-path)) | a full-width statement: **fleet state is unavailable — the store could not be read at 14:23:14**, over a floor that keeps its last state and is labelled *last known good*. On a cold start there is no floor to keep, and the screen says so in words | retry the snapshot with backoff; the stream stays open and will carry `fleet.health` | **an empty office.** This is [D2 § 8.6](FLEET-STATE.md#86-a-deliberately-invalid-exchange)'s forbidden outcome at the render layer |
| F5 | **`fleet.health` with `db: "down"` on connect** | the message D2 sends immediately on connect in that posture | the same statement as F4, and the stream's own indicator stays **connected** — the stream is up and is telling us why there is nothing | wait; D2 will send `fleet.health` again when `db` changes | rendering a connected socket as a healthy fleet |
...
| F21 | **The store becomes unreadable while the stream is open** — the mid-stream case, which is not F5's: F5 is the store being unreadable **at connect**. Reached two ways on one open stream — a tick's read of `feed_outbox` failing ([D2 § 2.2](FLEET-STATE.md#22-fail-posture-per-path)'s *feed stream, ALREADY OPEN* row) and the 15 s session re-check coming back ***unverifiable*** ([D2 § 9](FLEET-STATE.md#9-read-side-authentication)) — and **both render identically, because the viewer's situation is identical** | `fleet.health` with `db: "down"` arriving on a stream that is already open, rather than as its first message | **F5's, unchanged — this row states no render of its own and deliberately does not restate F5's**, because a second copy of one render is a copy that drifts. Nothing about what the viewer is looking at differs by *when* the store died: the stream's own indicator stays **connected**, and the floor keeps its last state labelled *last known good* | as F5 — **wait**. The stream is still the messenger: delivery resumes from the cursor it never advanced, and `fleet.health` with `db: "up"` arrives on the first tick whose read succeeds ([D2 § 2.2](FLEET-STATE.md#22-fail-posture-per-path)) | **rendering it as F6/F7** — a sign-in prompt over a dimmed floor. Nothing said the session was gone; the store could not be asked. That render replaces the one instrument still reporting the outage with a sign-in the viewer cannot complete, since authenticating reads the same store — and it is F5's own refused alternative with the sign flipped: not a connected socket rendered as a healthy fleet, but an unreadable store rendered as a revoked session |
...
that could not REACH the store is D2's third outcome, *unverifiable*, and ends nothing (F21) — which is the rule [§ 14](#14-open-questions-for-the-review-loop) item 5 asked for.
...
- **A tileset is not a map, and the absence below is untouched by this bullet.** What
  ...
  map is still card#7341's to author — the **shipped default** below, since card#9208's reversal — and
  `tools/design/verify-floor.py` still holds the absence because its sweep is for Tiled's two **map**
  spellings and a `.tsx` is neither.
...
- ⛔ **No floor map is vendored in this repository today, and the count above is therefore a figure
  this document declares rather than a measurement of a file.** The one map the repository ships is
  ...
  admits both and the choice between them stays the implementer's. **card#7341 authors it** — floor v1's map, drawn with the
  tileset the bullet above vendors, with the `docs/ATTRIBUTION.md` row Gate 1 requires. ⚠ **Under the
  reversed ruling this paragraph declared that artifact at the per-room path
  `resources/floor/aimla.tmj`; under this one it is the shipped default and lands at the path above.
  Nothing is renamed, because nothing exists at either path yet:** card#7341's tileset pull
  (PupFuzz/mezzanine#107, 2026-09-12) vendored the tileset and **no map** — the bullet above says so
  in terms — so the path moves before the file does, and the map card#7341 goes on to author is
  written to the default's path from the start. What keeps the two from drifting apart is the gate,
  ...
  map present while this paragraph still declares the absence is its CONTRADICTED branch — so a
  floor-v1 pull that lands `aimla.tmj` reds by name and says which sentence it disagrees with. While
  no map exists the gate requires this paragraph to say so and reds if a map appears anywhere in the trees this repository authors (the gate
  prints the trees it skips); the day one lands at the declared path it counts the objects of the
  `desks` layer and reds if that count and `S` disagree; and in both states it requires this section to
  ...
  it — the inverse of the check it held while the ruling said there was none. Which is the point of
  writing the absence down: until card#9208 this document spoke of *the shipped map* while none was
...
⭐ **The in-browser editor is NOT built, and it is priced rather than left as an implication.** The
...
that the store, the validator, the revisions and the table above are untouched by it.
[§ 14](#14-open-questions-for-the-review-loop) item 18 carries it as the operator's call.
...
| REST poll while the feed is down | 10 s | **Cited** — [D2 § 2.2](FLEET-STATE.md#22-fail-posture-per-path); since card#9287 also the cadence the client re-opens the stream on ([§ 2.2](#22-connect-snapshot-deltas)) | [§ 9](#9-failure-paths-and-their-observables) |
...
| Worst-case delta at the cap of 8 | **6,169 B** | **Cited** — D2 § 8.2.1, measured by serializing [D2 § 8.3.2](FLEET-STATE.md#832-worked-worst-case-delta) | [§ 8.1](#81-the-cap-stays-at-8--the-arithmetic-and-the-reason) |
| Feed message bound | **8,192 B** | **Cited** — D2 § 8.3 | [§ 8.1](#81-the-cap-stays-at-8--the-arithmetic-and-the-reason) |
| Spare under the bound | **2,023 B** | **Derived** — 8,192 − 6,169 | [§ 8.1](#81-the-cap-stays-at-8--the-arithmetic-and-the-reason) |
...
| Further elements that fit | **7** | **Derived** — ⌊2,023 ÷ 263⌋ | [§ 8.1](#81-the-cap-stays-at-8--the-arithmetic-and-the-reason) |
| The cap could reach / its worst case | **15** / **8,010 B** | **Derived** — 8 + 7; 6,169 + 7 × 263 | [§ 8.1](#81-the-cap-stays-at-8--the-arithmetic-and-the-reason) |
| A cap of 16 breaches by | **8,273 B**, 81 B over | **Derived** — 8,010 + 263 against 8,192 | [§ 8.1](#81-the-cap-stays-at-8--the-arithmetic-and-the-reason) |
...
| Desk slots the shipped default map declares | **12** | **Chosen** — 3× the `aimla` room's four seats (`docs/PLAN.md § 5`'s rollout order), which leaves room for the Windows validation seat and the next few without an edit; an authored room's `S` is whatever its author drew, read from the document and never from this row. ⚠ **Chosen, not measured, and it stays that way until the default is a file**: the gate holds the figure against whichever of the two states [§ 10.3](#103-the-floor-map) declares — the default's ABSENCE while there is none, its `desks` layer once there is | [§ 3.2](#32-the-desk-slot-function) |
...
| **G3 cap arithmetic** | 6,169 / 8,192 / 263 / 2,023 / 7 / 15 / 8,010 / 8,273 / 81 re-computed from the **three** inputs (worst case, bound, per-element), and those three checked for **presence in D2** — anywhere in D2, not at a named statement, which is the narrower claim the tool can actually make and is why "is a Cited number true at its D2 home" stays on the hand-verified rows below | **tool-checked** |
...
| 33 | **card#9269 is re-scoped: it owes the `office` form's INTERIOR and not the hallway** ([§ 4.6](#46-the-building-layout)) | absorb card#9269 into this design; or leave both cards standing with the hallway on each | Two cards designing one hallway is the two-homes defect minted onto the tracker. The office's interior is a room map, which is that card's unit and not this one's; the hallway is the floor's, which is this one's. Absorbing the card would put a room map's authoring on a card about the floor above it | **What card#9269 must still settle is left open and named, not decided here:** whether the `office` form selects a **second** shipped default, against [D2 § 13](FLEET-STATE.md#13-decisions-taken-revisable-at-review) row 44's one-default rule. This design assumes row 44 as written — an unauthored room of either form renders the one default — and a planned floor's overlap check reads whatever grid [D2 § 8.7](FLEET-STATE.md#87-the-building-surface--the-layout-the-room-maps-and-the-message-that-says-one-changed) answers for the room, so it holds under either answer |
...
| 35 | **The client owns reconnect: an errored `EventSource` is closed and re-opened on the 10 s poll cadence, never left to the browser's built-in retry** ([§ 2.2](#22-connect-snapshot-deltas)) | let `EventSource` reconnect itself at the browser's default; ask D2 to write a `retry:` field | The browser default is three browsers' three numbers, D2's primitive writes no `retry:`, and a host refusing streams (F20) would be hit at that default by every tab. One cadence the document already owns is one number to reason about, and a `feed.close` carrying `session` must re-open nothing, which the built-in retry cannot be told | a dropped stream is back in ten seconds rather than three; if that ever matters, the change is one constant here and a `retry:` line at D2 |
| 36 | **F7 is closed at D2 and the ≤ 15 s enforcement window is accepted, not hedged client-side** ([§ 9](#9-failure-paths-and-their-observables)) | keep F7 as a residual; guess expiry client-side from the session's known lifetime and dim the floor pre-emptively | D2 § 9's re-check is the rule item 5 asked for, and a client-side guess renders *not live* on a floor whose stream the server is still happily serving — a lie in the safe direction is still a lie, and this document's whole method is that the render follows a delivered fact | a viewer sees up to 15 s of a fleet its session is no longer entitled to; the number is D2's and one tick, and shortening it is a D2 change |
...
5. **✅ CLOSED — D2 § 9 re-checks the stream's session every 15 s (card#9287).** This item asked what
   ...
   the session or its MFA is gone. (Card#9287 later gave that check a third outcome, *unverifiable* — the
   store could not be reached — which ends nothing and renders as F21.)
...
18. **⇢ Operator — an in-browser room editor.** Opened by card#9208's reversal (2026-09-12), which
    made rooms operator-authored at runtime and left Tiled as the editor
    ([§ 10.3](#103-the-floor-map), [§ 13](#13-decisions-taken-revisable-at-review) row 28). **Blocks:**
    nothing — an operator with Tiled installed can author every room today. **In the meantime:** the
    console validates, previews, revisions, diffs, restores and exports; the editing is Tiled's.
    **Closes it:** an operator call on whether the console should paint tiles itself, priced at weeks
    of front-end work — a palette from the shipped tileset, layers, a desk-slot tool, undo — under one
    constraint stated now so it is not discovered: it **emits the same Tiled document** § 10.3's table
    reads, a second writer of one format and never a second format, so the store, the validator and
    the revisions are untouched by it.
