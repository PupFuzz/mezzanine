...
are rendered in an explicitly labelled **overflow row** below the floor, its label reading
***Overflow — seats past the map's desks*** (ratified by the operator on card#7342, 2026-09-25) — same desk, same render, same
...
**The seat segment's two notices**, each the facts in order and nothing else (ratified by the operator
on card#7342, 2026-09-25): a `seat_id` naming no desk on the floor reads ***no desk `seat_id` on this
floor***, and one naming desks in more than one room reads ***the seat `seat_id` names a desk in more
...
the cross-section PR #96 built. Until row 15 lands, the floor page draws the room at the page's width
under no camera (row 14's slice A) with row 8's text render of the desks below it, at every viewport
size; row 15 replaces that page-side text render with **the list view** the first rule
...
| clock skew | `delivery.clock_skew_ms` | `412` | **`fetch-fresh`**; rendered whenever non-null, beside **every** seat-clock timestamp in the panel, so a narrative time is never read as an absolute one, worded ***seat clock is ±N ms from the server's*** — `N` signed and in the wire's own unit (ratified by the operator on card#7342, 2026-09-25); a null skew is no note at all, never *0 ms*. The `clock_skew` **badge** past ±120 s is version-bearing and is what carries the condition onto the desk ([§ 7.2](#72-badges-every-member-has-a-render)) |
...
| `action.agent_scope` | no *this is a subagent's call* marker. Never defaulted to `main`: a defaulted scope would attribute a call to the seat's main agent or to an intern on a field the wire left empty. The drill-down's intern list does not read this field — it selects on the dispatch calls ([§ 5.2](#52-the-drill-down)) |
...
1. **⇢ D2 — the drill-down's two endpoints carry no field tables.**
   ...
   **The reading this document took**, stated because this section's own preamble requires it:
   ...
   drill-down's intern list is the subset that are **dispatches** —
   [D2 § 6.4](FLEET-STATE.md#64-ddl)'s `calls.is_dispatch` (operator ruling 2026-09-25, card#7342,
   which replaced a selection on `agent_scope == "subagent"` / a non-null `parent_call_id`: those
   labels attribute an intern's own calls back to it, the intern join
   [D2 § 4.8](FLEET-STATE.md#48-what-may-never-mint-a-state) stores them for). If the field table
   answers otherwise, the list's selection predicate changes and nothing else in this document does.
