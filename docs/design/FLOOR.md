...
| `protocol_agent_name` | the seat joins **nothing**: no coordination participant resolves to this desk, and the desk itself renders exactly as it did before — no marker, no *(no agent)* label, and never `seat_id` in its place ([§ 5.7](#57-the-coordination-thread-line)). Null is *this seat declares no protocol agent name*, or *no heartbeat has arrived yet*; the member below is what tells those apart |
| `protocol_agent_name_check` | nothing is drawn, and the seat resolves nothing. Null is **not** read as `undeclared`: `undeclared` is a seat that reported and declared none, null is a seat that has not reported at all — the same three-value distinction `enabled` carries one row down, for the same reason. The state is rendered only beside a participant this seat resolves ([§ 5.7](#57-the-coordination-thread-line)), so a seat resolving nothing renders none of it |
