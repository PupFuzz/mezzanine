...
29. **⇢ Review — an exit caused by a failed read has no object that ended the hold, and this document
    ...
    `missing_persistent`, [§ 11](#11-acceptance-tests)'s fixture table), AT-D3-1 does not replay it, and no test is loosened. ⚠ **The ordering stated above was not met.** Row 15's list-view guard (slice A,
    `Tests\Feature\Floor\TheListViewRendersEveryDeskMemberTest`) now replays that run through the desk render,
    and the question is still unanswered. That is harmless, and the reason is what the guard reads: only the desk
    models it draws, never the animation log, so it asserts nothing about this exit's `cause` — no test asserts on
    the log over that run, and AT-D3-1 still excludes it. The trigger is therefore re-scoped to the one that
    matters: the rule is owed before any test asserts on the animation log over that run. The owner is unchanged —
    [Appendix B](#appendix-b--what-an-implementer-builds-from-this) row 15, now its slice B. Ruled by the maintainer, card#7341 (review of
    PupFuzz/mezzanine#238 r1).
