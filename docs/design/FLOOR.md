...
decision. [Appendix B](#appendix-b--what-an-implementer-builds-from-this) row 14 paints the room on it, and below the drawing each desk is still a line of text until row 15 moves
the camera over the room and replaces that text render with [§ 4.5](#45-the-viewport-rule-and-the-capability-floor)'s list view — a headless model over every fact the desk model emits, which the page only paints below the viewport floor. ⭐ **The drill-down form, `/floor/{floor}/{seat_id}`, is served** — [Appendix
...
|---|---|---|
| *a hallway with 5 office rooms for solo agents* | five installs, `sola` … `zeta`, each `office`; each map a 14 × 8 grid of 32 px tiles — 448 × 256 px — with one `desks` object at [§ 12](#12-every-number-and-where-it-comes-from)'s furniture box, 440 × 228 px (a room holds at least one desk at the box, [§ 14](#14-open-questions-for-the-review-loop) item 28(1)) | `rooms`: the *i*-th office at `{x: 480·i, y: 160}` — 32 px between neighbours, the plan's own spacing, which [§ 12](#12-every-number-and-where-it-comes-from)'s default gap does not govern; `hallway`: a 75 × 5 grid of corridor tiles, 2,400 × 160 px, along the top. The floor's extent is their union, 2,400 × 416 px; the corridor is scenery and the offices' own walls are their maps' |
| *a big room for PM+3impl agents and a small room for PM+1impl agent* | `aimla`, four seats, a 42 × 15 grid of 32 px tiles — 1,344 × 480 px — with `S = 6`, two rows of three desks at the furniture box; `beta`, two seats, a 42 × 8 grid of the same tiles — 1,344 × 256 px — with `S = 3`, one row of three | `rooms`: `aimla` at `{x: 0, y: 0}`, `beta` at `{x: 1408, y: 0}`; no `hallway` — the gap between them is the plan's own and is the floor's bare background. *Big* and *small* are the maps' grids; the entry says only which is left. A fifth `aimla` seat is [§ 3.2](#32-the-desk-slot-function)'s overflow row, plan or no plan |
...
28. **⇢ Review — the room is drawn and the camera built at [Appendix B](#appendix-b--what-an-implementer-builds-from-this) rows 14–16, and what stays open
    ...
    *(1)* ✅ **RULED 2026-09-25 — operator, card#7341 comment 6488: *yes to all three*, (i), (ii) and (iii) as
    ...
    are [Appendix B](#appendix-b--what-an-implementer-builds-from-this) row 14's, built by its slice C on 2026-09-25 — that row records how, and that a revision
    stored before it is re-validated and listed if it fails, never assumed passing — and the record below stands as the question was
