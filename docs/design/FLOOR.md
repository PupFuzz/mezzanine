...
the camera over it; that text render then stays, widened at row 15 to every fact the desk model emits, as [§ 4.5](#45-the-viewport-rule-and-the-capability-floor)'s list view rather than being deleted. ⚠ The drill-down form, `/floor/{floor}/{seat_id}`, is still not served: the panel is step
10's.
...
| ***desk slots `i` and `j` intersect — `install_id`***, one per pair, and ***desk slot `i` is smaller than the furniture box — `install_id`***, one per slot | two `desks` objects of a map the client holds sharing a pixel on half-open rects, or one object smaller than [§ 12](#12-every-number-and-where-it-comes-from)'s furniture box at the cap — the object ids in `id` order, the room's suffix as [§ 4.6](#46-the-building-layout)'s layout names it ([§ 10.3](#103-the-floor-map), [§ 9](#9-failure-paths-and-their-observables) F21) | a fact about the map and this client's layout, not about any seat. It is rendered for **any** map the client holds — the shipped default, an authored save, a stored or restored revision — because the console refuses neither defect yet ([§ 14](#14-open-questions-for-the-review-loop) item 28) and a refusal at the write would not reach a revision stored before it; so this line, and not a harness test, is what tells a viewer and the map's author, and it is drawn by [Appendix B](#appendix-b--what-an-implementer-builds-from-this) row 14's scene from the map alone |
...
default by `tools/design/verify-floor.py` G8, and for an authored map by nothing yet — [§ 14](#14-open-questions-for-the-review-loop) item 28's
refusal is open, and [§ 9](#9-failure-paths-and-their-observables) F21's notice is what stands meanwhile) and on row 14 (everything the scene draws for a
...
- **GREEN:** *(a)* every desk's **furniture box** — the union rect of everything the scene draws for the desk
  except the bubble, at the stools and badges it carries — lies inside its slot rect, and every two desks'
  ...
  *(a)*'s containment is read against it; *(d)* every bubble's box is sized from the measurer's answer and
  ...
  past the edge, and every one of the cap's stools and the *+N more* tag are drawn, none hidden
  ([§ 10.3](#103-the-floor-map)'s bounds); *(f)* on the crowded map the scene emits
  F21's ***desk slots `i` and `j` intersect*** naming the two slots and the room, and both desks are drawn,
  the later `id` on top, each inside its own slot; on the undersized map it emits ***desk slot `i` is smaller
  than the furniture box*** naming the slot and the room, and that desk is drawn at native size from its
  anchor past its edge, neither scaled nor clipped, every stool drawn; and on every other run it emits
  neither line.
- **RED — the tray outside the slot:** hang the side table beside the slot rect, as the reference's `deskSVG`
  ...
  its neighbour's. The cap leg is kept for the worst case — eight stools and the tag — which is the width the
  box is measured at.
- **Second RED — the string at its width:** draw the descriptor, or a stool's label, at the width the measurer
  answers for the whole string → on the cap leg it crosses the slot's edge, *(a)* fails, and *(e)* fails on the
  missing mark; the desk drawn past its slot is the reference's tray defect arriving through text instead of
  furniture, which is why the cap leg carries every string at its bound.
...
- **Fourth RED — the silent crowded map:** draw the two desks over each other and emit no line → *(f)* fails;
  ...
  [§ 9](#9-failure-paths-and-their-observables)'s opening sentence names. Watched because the console refuses nothing for it ([§ 14](#14-open-questions-for-the-review-loop) item 28),
...
- **GREEN — the floor half:** after the wheel-zoom the scene point that was under the cursor is still under
  ...
  on `fx-degraded` the `fold_lag` seat's row carries the lag line [AT-D3-5](#at-d3-5-a-degraded-seat-is-visibly-degraded) asserts and the `catching_up`
  seat's row its currency label — and on growing back draws the floor again at fit, with no row for that either.
...
28. **⇢ Review — the room is drawn and the camera built at [Appendix B](#appendix-b--what-an-implementer-builds-from-this) rows 14–16, and three things stay open
    past them.** Doc-owner design, card#7341, raised by the step-8 builder (PR #226): rows 0–13 built every
    ...
    *(1)* **The console refuses no intersecting `desks` objects, and no object too small for the furniture
    ...
    old document; and (iii) for the revisions already stored under the old meaning when row 14 lands, should
    the console re-validate every room's current map at that landing and list the failing rooms, each left
    on the floor under F21's notice until its author saves a passing one?** Recommendation: yes to (i) and
    (ii) at both writes, landing with row 14's build — the step that gives the console's preview the renderer
    — because a refusal at the write reaches the author before a viewer does, which is § 4.6's own argument
    for every other refusal in that table; and yes to (iii) as a listing and never a refusal, because a
    stored document cannot be refused after the fact without blanking its room, which is F16's *Never*, and
    F21's notice stands on the floor meanwhile whatever the answer.
