...
the camera over it; row 15 then replaces that text render with [§ 4.5](#45-the-viewport-rule-and-the-capability-floor)'s list view — a headless model over every fact the desk model emits, which the page only paints below the viewport floor. ⚠ The drill-down form, `/floor/{floor}/{seat_id}`, is still not served: the panel is step
10's.
...
- **Second RED — the primitive that does not cut:** make the scene's one truncation primitive return its
  ...
  through it ([§ 10.3](#103-the-floor-map)): one plant covers the strings the fixture does not bound, where a
  per-site plant would prove only the sites the fixture happens to stretch. The desk drawn past its slot is
  the reference's tray defect arriving through text instead of furniture.
...
**A third note on order — rows 14, 15 and 16, and why they carry those numbers.** They are the room drawing, the camera and the building cross-section (designed in card#7341's round after step 8; rows 14 and 15 build under card#7341 and row 16 under card#7343), and their dependency order is the one each row states, not the number: 14 draws over row 8's page and steps 5–7's models; 15 moves what 14 draws; 16 needs 15 and row 9's lobby. Relative to row 9, rows 14 and 15 are independent of it — they touch the floor page alone — and 16 comes after it. Relative to row 10, the build order is free and the product is not: the drill-down is a panel over the floor that opens on a selected desk and by its own route ([§ 4.4](#44-routes-and-what-each-one-fetches)), so a panel that lands first opens from the list view's row and from the route, and row 14 then gives it a drawn desk to open on; either order is legal and the Gate cells are the truth. ⚠ **But row 14 depends on row 10 for a surface, not for a gate:** [§ 10.3](#103-the-floor-map) truncates every desk string to the furniture box and puts the full value on the list view and in the drill-down — and above the viewport floor the list view is not rendered, so on a desktop the drill-down is the only full-value surface. Until row 10 lands, a truncated string's full value is reachable only below the floor; that gap is stated here rather than papered over with an interim surface, and row 10 is in build on its own branch, so the two rows land together or row 10 first. They are numbered 14–16 rather than 8a–8c because the number is what G5 compares — a gate at step N reading an artifact of step M holds iff N ≥ M — and an Order cell that is not an integer is a row the check reds rather than reads (`tools/design/verify-floor.py`, since this change); rows 11–13, which rows 7 and 9 read from, already sit after them for the same reason, so a row's number says when it was written and its cell says what it needs.
