...
- **The floor requires ≥ 1,280 × 800 CSS px.** Below that, the route renders the **floor page's list
  ...
  the seat ([§ 5.1](#51-the-desk)'s rows and [§ 7](#7-degradation--how-a-degraded-seat-is-unmistakable)'s marks — the lag line, the config note, the currency label,
  the open-call count, the action and its elapsed, the gauge, the model label, the oldest-badge stamp,
  the side table's stools and the bubble's text — each as the string the model already decided),
  because a list that dropped § 7's marks would be the one render a small viewport gets showing a
  degraded seat as healthy. A scaled-down floor whose
...
viewport size (row 8), and that text render becomes **the list view** the first rule names once row 15
widens it to every fact the desk model emits — it is kept and widened, never replaced — and row 15
...
| `fx-interns` | one seat whose `subagents` goes 0 → 8 → 8-with-`subagents_open`-9, including one element with `title: null`; and **the cap leg** — `fx-snapshot-4` with that array at the cap on both `aimla-impl-2` and `aimla-impl-1`, whose slots in [§ 3.2](#32-the-desk-slot-function)'s worked assignment are neighbours on the shipped default's `desks` layer, so a seat at the cap sits beside an occupied slot that is itself at the cap ([AT-D3-20](#at-d3-20-seat-furniture-never-overlaps-and-the-overflow-row-stays-below-the-floor)'s leg; the harness asserts the two slot rects share an edge, so a re-authored default that parts them reds the precondition rather than passing on nothing) |
...
default by `tools/design/verify-floor.py` G8, and for an authored map by the console once [§ 14](#14-open-questions-for-the-review-loop) item 28's
refusal lands) and on row 14 (everything the scene draws for a desk lies INSIDE its slot rect, at [§ 8](#8-interns--subagent-rendering-and-the-cap)'s cap
of stools). This test holds the last, which is the scene's and the one no map check can see; the bubble is
[§ 5.1](#51-the-desk) rule 5's own pass over the base rects and is checked as one. ⚠ The ratified reference is NOT the
...
- **Build:** replay `fx-snapshot-4` on the shipped default map; then `fx-interns`' cap leg on the same map — a
  seat at the cap whose slot rect shares an edge with an occupied slot's, itself at the cap; then `fx-snapshot-4`
  on a map the harness stubs to two `desks` objects, so two seats overflow; in all, every seat carrying a
  `task.title` at [D2 § 8.2.1](FLEET-STATE.md#821-the-seat-state-object)'s bound, and a measurer that answers a stated width per glyph. Read the
  scene's rects, then read them again with the seats delivered in a different order. **Reads:** **the
  harness**, the **scene**, the **floor layout**.
- **GREEN:** *(a)* every desk's **furniture box** — the union rect of its sprite, character, nameplate, badge
  cluster and side table, at the stools it carries — lies inside its slot rect, and every two desks' furniture
  boxes are disjoint; intersection is on **half-open** rects, `[x, x + w) × [y, y + h)`, so a shared edge is
  not an intersection ([§ 4.6](#46-the-building-layout)'s footprints, F18); on the cap leg both cap desks satisfy it, which is the case
  the reference's tray failed; *(b)* no bubble intersects another desk's furniture box, another bubble, the
  wall clock's face, or the overflow strip's own header, and the second reading places every bubble exactly
  where the first did — rule 5's pass over the base rects in a fixed order, so two browsers rendering one
  fleet agree with nothing stored ([§ 3.1](#31-the-keys-and-why-they-are-the-only-ones)); *(c)* on the overflowing run every overflow desk's furniture
  box and bubble lie below the floor's extent and inside the strip's box, and no furniture of the floor
  reaches below the extent — one scalar per floor in each direction, which is the boundary the reference's
  check states; *(d)* every bubble's box is sized from the measurer's answer and the title at the bound is
  drawn truncated with a mark (rule 4).
- **RED — the tray outside the slot:** hang the side table beside the slot rect, as the reference's `deskSVG`
  does → on the cap leg *(a)* fails twice, the cap desk's box leaving its rect and meeting its neighbour's.
  `fx-snapshot-4` alone would not catch it — its one seat with a stool sits beside an empty slot, and the only
  adjacent occupied pair carries none — which is why the cap leg exists.
- **Second RED — the crowded map:** stub a `desks` layer whose two slot rects intersect → *(a)* fails naming
  both seats. Watched, because the console refuses nothing for it ([§ 14](#14-open-questions-for-the-review-loop) item 28): this test is what tells a
  map author, and a gate that could not fail on the one map defect the write does not catch would be a
  decoration.
- **Third RED — the order-dependent bubble:** resolve each bubble against already-displaced neighbours →
  the second reading places a bubble elsewhere, and two browsers on one fleet draw two floors.
- **Fourth RED — the strip on the floor:** draw the overflow row on the floor's own last band, which is where
  the reference drew it until card#7965 → *(c)* fails on the first overflow desk.
- **Fifth RED — the fixed-width bubble:** size the bubble from a constant → the title at the bound is clipped
...
- **Discriminating control:** the shipped default map with the four `fx-snapshot-4` seats passes every clause,
  so the gate is known to be able to say *nothing overlaps*; and the cap leg on that map passes only once row
  14 has re-authored its objects to the furniture box — on the sprite-sized objects it ships today the cap
  desk cannot lie inside its rect and *(a)* fails on it, which is the measurement that says the re-authoring
...
- **Build — the floor half:** replay `fx-snapshot-4` through the scene at a viewport the fixture states —
  ...
  **camera**, the **capability floor**, the **scene**, the **animation log**.
- **GREEN — the floor half:** after the wheel-zoom the scene point that was under the cursor is still under
  ...
  per seat, no map, every fact the desk model emits: on `fx-degraded` the `fold_lag` seat's row carries the
  lag line [AT-D3-5](#at-d3-5-a-degraded-seat-is-visibly-degraded) asserts and the `catching_up` seat's row its currency label — and on growing back draws the floor again at fit, with no row for that either.
...
- **Fifth RED — the healthy list:** drop the lag line from the list view, as row 8's text render did → below
  the floor the `fold_lag` seat reads as its state alone, and [§ 7](#7-degradation--how-a-degraded-seat-is-unmistakable)'s *a degraded seat is unmistakable* fails
...
28. **⇢ Review — the room is drawn and the camera built at [Appendix B](#appendix-b--what-an-implementer-builds-from-this) rows 14–16, and three things stay open
    ...
    *(1)* **The console refuses no intersecting `desks` objects, and no object too small for the furniture
    ...
    Meanwhile: `tools/design/verify-floor.py` G8 holds the shipped default pairwise disjoint on every run,
    [AT-D3-20](#at-d3-20-seat-furniture-never-overlaps-and-the-overflow-row-stays-below-the-floor)'s second RED is the authored case, and an authored object smaller than the box is drawn at
    native size from its anchor and past its edge — neither scaled ([§ 13](#13-decisions-taken-revisable-at-review) row 14's refused alternative) nor
    clipped (a hidden stool is a dropped intern) — with AT-D3-20's *(a)* as what tells the author. What would
    close it is a refusal row per case in § 10.3's table, each a change to what the console accepts and so
    the operator's to rule: ⇢ **Operator — should the console refuse (i) two `desks` objects whose half-open
    rects intersect, naming both, and (ii) a `desks` object smaller than the furniture box at the cap, the
    size [§ 12](#12-every-number-and-where-it-comes-from) gains as a Measured row when row 14 lands?** Recommendation: yes to both, landing with
    row 14's build — the step that gives the console's preview the renderer — because a refusal at the write
    reaches the author before a viewer does, which is § 4.6's own argument for every other refusal in that
    table.
