...
- **`held`** — a render **held** for exactly as long as a delivered field has a value: the working,
  ...
  row 4's backstop, or a condition the client holds itself while the held object still says to draw the
  render — [§ 11](#11-acceptance-tests) owns what the exit records and why such an exit takes no row of
  this table, and it owns the episode too, including whether a change of `motion` alone ends one.
...
continuous run of one render at one `motion` on one seat: the log mints a fresh `episode_id` each time a renderer starts an animation or enters a held
...
instant of that render — whose `cause` is the literal naming the condition:

| The condition | What the desk does | The `left` row's `cause` |
|---|---|---|
| [§ 2.3](#23-membership-a-seat-or-an-install-the-client-does-not-hold) row 5: a failed read takes the seat's run of consecutive refused reads to that row's threshold | draws the empty chair, so its render is left and nothing is entered | **`unconfirmed`** |
...
- **Neither is an exit of a new kind, and neither takes a row in
  [§ 6.2](#62-the-animation-table--the-closed-set)'s closed set.** Neither class admits one. An `edge`
  row needs a wire message the client applied, and a refused read or an ended session is a response it
  could not apply; a `held` row needs a delivered field holding it, and what holds the chair is the
  client's own count of refused reads, and what holds a stilled floor still is the client's own
  knowledge that its session has ended. So neither change enters anything of its own: the chair enters
  nothing, exactly as a `stale` or `offline` chair enters nothing, and a stilled desk re-enters only the
  row its delivered field still holds. [§ 6.5](#65-a-snapshot-never-animates)'s reasoning lands on the
  same answer from the other side: an emptied or stilled desk is a claim about what the client knows,
  not about anything the seat did, so nothing about it moves.
- **Each literal names the condition the client observed, and not one event of it.** No single refused
  read ended the hold — the first leaves the desk drawn, which is row 5's threshold's whole purpose — and
  what stills the floor is the session having ended, not the read that reported it, since F7 reaches the
  same render with no read at all. Each literal is the name the client already gives its condition — the desk model's
  `unconfirmed`, the desk floor's `stilled` — and each is row 4's `snapshot` one case over: a literal
  wherever something the client observed ended a hold and no object did. **Where both conditions arise
  in one render**, the row names the one that ended what the episode held: a desk drawn as the empty
  chair draws no render, so it has no loop left to still, and its row is `unconfirmed`.
- **Only where no object ended it.** Renders are coalesced — one runs at a time, and everything that
...
| `fx-confirm` | ✅ landed with step 3 (`server/tests/Feature/Floor/fixtures/fx-confirm.json`, replayed by `Tests\Feature\Floor\TheClientProtocolReplaysTheSameWayEveryTimeTest`) and declared here since PR #227's design round: [§ 2.3](#23-membership-a-seat-or-an-install-the-client-does-not-hold) row 5's runs, each a seat the client holds whose confirming read is refused. **`missing_persistent` is the unconfirmed desk's run** — a snapshot, then every read of one held seat refused past row 5's threshold of refused reads, the stream carrying on — the one run in which the desk model's `unconfirmed` is true, which no snapshot can make it. The held render that exits there is ended by a failed read and not by an object, and [§ 11](#11-acceptance-tests)'s rule for a hold a client-side condition ended is what that exit records ([§ 14](#14-open-questions-for-the-review-loop) item 29, closed). ⛔ [AT-D3-1](#at-d3-1-no-animation-without-its-event) does not replay the run yet, and two things stand in the way, both [Appendix B](#appendix-b--what-an-implementer-builds-from-this) row 15's to fix. **The client:** it writes that exit against the held object's own `state_version` rather than the rule's literal. **This fixture:** every refused read in the run answers a `503` body carrying the snapshot's own `server_time`, `2025-08-23T14:23:20.000Z`, and [§ 2.4](#24-the-clock-and-every-age-on-the-page)'s offset is refreshed from each, so every refusal pulls the corrected clock back to the snapshot's instant and the exit is dated the instant its episode was entered — AT-D3-1's `left.at > entered.at` reds on a correct client until each `503` body carries the `server_time` of the instant it answers, on the run's own clock. No test is loosened to admit either. A fixture sets values; it renders none, so this row is **`named-not-rendered`** |
...
- **Build — the closed-set half:** replay `fx-snapshot-4`, `fx-clear-trace`, `fx-degraded` and
  `fx-interns` end to end; collect the animation log. ⚠ Not yet `fx-refusals`' `refusal_401_warm` run,
  whose `401` stills a floor with loops running — its exits are the second condition of the same rule
  below, and the run joins this half with the client fix for that condition. ⚠ Not yet `fx-confirm`'s
  `missing_persistent` run: the held render that exits there is ended by a failed read and not by an
  object, and
  [§ 11](#11-acceptance-tests)'s rule for a hold a client-side condition ended says what that exit
  records and what this test holds it to. The run joins this half when the client writes that exit
  **and** the fixture's refused reads carry a `server_time` that advances, both of which are
  [Appendix B](#appendix-b--what-an-implementer-builds-from-this) row 15's to deliver (the
  [fixture table](#11-acceptance-tests)'s `fx-confirm` row states where the client and the fixture
  stand); until then it is left out, and no assertion here is loosened to admit it. **Reads:** the **animation log**, **the harness**, the
  **animation set**.
- **GREEN — the closed-set half:** every `animation_id` in the log is a row of
  ...
  of that type); **every `held` row's `cause` is what [§ 11](#11-acceptance-tests)'s schema states for
  its phase** — the `cause` column, and for the coordination rows the paragraph under it that states
  theirs — and the two phases are asserted in **opposite** directions, which is what makes both
...
21. **✅ CLOSED — the version base is `fx-snapshot-4`'s own `state_version`; pm's still-open Bash call
    ...
    **48229** across `fx-clear-trace`'s ten deltas (D2 § 10's own E0…E9, with `E0` carrying the
    ...
    ("that seat's `state_version`", "what § 11's schema states for its phase") — so this costs nothing to any
...
29. **✅ CLOSED — a hold that a condition the client holds itself ended, and no object did, is left with
    ...
    is no exit of a new kind and takes no row in [§ 6.2](#62-the-animation-table--the-closed-set).**
    Doc-owner ruling, card#7341. Found at PR #227's round 4 (card#7341's rows 14–16 design round):
    [§ 2.3](#23-membership-a-seat-or-an-install-the-client-does-not-hold) row 5 makes a held seat the
    client cannot confirm render the empty chair, so a `held` render open on that desk exits;
    [§ 6.2](#62-the-animation-table--the-closed-set) recorded an exit against the object that ended the
    hold, and [§ 11](#11-acceptance-tests) required a `held` row's `cause` to be a `state_version` the
    client holds and a `left` row's object to no longer satisfy the hold condition — but a failed read
    advances no version and changes no object, so
    ...
    **The audited sibling, and why the ruling is a class and not row 5's alone** (PR #240 review r1).
    [§ 9](#9-failure-paths-and-their-observables) F6 is the same shape. Once any read returns `401`, the
    desk floor marks every desk `stilled` (`desk/desk-floor.js`, `renderWith()`), the desk render then
    withholds permission to run a loop (`desk/desk-render.js`, `permitted`), and the animation set
    leaves every episode whose `motion` changed and writes the held object's own `state_version` as its
    `cause` (`wire/animation-set.js`, `held()`). No object ended those holds: the `cause` is the entering
    row's own version, and the object still satisfies the row's condition. It is reachable on a
    checked-in run. Replaying `fx-refusals`' `refusal_401_warm` run through
    `server/tests/Feature/Floor/fleet-client-probe.mjs` on the shipped client, each of the four
    `fx-snapshot-4` desks writes a `left` row whose `cause` equals its `entered` row's, followed by an
