...
served today** (card#9208's build slices, [Appendix B](#appendix-b--what-an-implementer-builds-from-this)),
so this is what the route must do when it is written rather than a behaviour anything exercises now.
...
[§ 4.4](#44-routes-and-what-each-one-fetches)'s floor route as a SERVED route — no page holds it, and
no page constructs the client protocol before [Appendix B](#appendix-b--what-an-implementer-builds-from-this)
step 8 — so a composed floor is a plate in the lobby and not yet a screen a viewer can open. ⭐ **What
...
therefore built except for the page that mounts it; and the office map a room's `office` form will select — card#9269, **re-scoped by card#9292
