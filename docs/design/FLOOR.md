...
deep-link on a cold start ([Appendix B](#appendix-b--what-an-implementer-builds-from-this) row 16 draws the cross-section and the ride; row 15 builds the camera). **A cross-section that had replaced the summary with the desks themselves
...
the camera over it; that text render then stays as [§ 4.5](#45-the-viewport-rule-and-the-capability-floor)'s list view rather than being deleted. ⚠ The drill-down form, `/floor/{floor}/{seat_id}`, is still not served: the panel is step
10's.
...
capability floor at row 15, over the room drawing row 14 paints, and the building cross-section at row
16. Until row 15 lands, the floor page draws the desks as text at every viewport size (row 8), and that
text render **is the list view** the first rule names — it is kept, not replaced, and row 15 makes it the
below-the-floor render. The camera is one machinery at two scales, and the second scale is reached by a
...
**Fixtures.** One table, shared by the tests below; its row count is re-derived by `tools/design/verify-floor.py` G5 on every run, against every test's use of it in both directions, and no figure is written here (the one that was read *nine* over ten rows):
...
- **Build:** replay `fx-snapshot-4` with the scene told — as the painter tells it — that the room's tileset
  image failed to load; then, from an intact floor, that one seat's character art failed; then the same
...
- **GREEN:** with the tileset failed, every desk of that room draws the placeholder — a plain rectangle
  ...
  ([§ 2.5](#25-what-re-renders-and-when) has no row for one) and a desk keeps its `render_state` through it; a reload requests the art again,
  which is the recovery F14 names.
...
for, gated at [Appendix B](#appendix-b--what-an-implementer-builds-from-this) **step 14**. The construction is two halves and this test holds the one the map cannot:
distinct seats take distinct slots ([§ 3.2](#32-the-desk-slot-function)) and the slots are the map's own rects ([§ 10.3](#103-the-floor-map)), so what is left
to check is that what is drawn AROUND a slot stays clear of its neighbours, and that [§ 5.1](#51-the-desk) rule 5's bubble
separation actually separates. Read from the scene's emitted rects, which is the check the reference
artifact's own node selftest runs on that artifact and for the same reason: no browser on the build host.*

- **Build:** replay `fx-snapshot-4` on the shipped default map; then on a map the harness stubs to two `desks`
  objects, so two seats overflow; in both, every seat carrying a `task.title` at [D2 § 8.2.1](FLEET-STATE.md#821-the-seat-state-object)'s bound, and a
  measurer that answers a stated width per glyph. Read the scene's rects, then read them again with the
  seats delivered in a different order. **Reads:** **the harness**, the **scene**, the **floor layout**.
- **GREEN:** *(a)* every two desks' **furniture boxes** — the union rect of a desk's sprite, character,
  nameplate, badge cluster and side table — are disjoint; *(b)* no bubble intersects another desk's furniture
  box, another bubble, the wall clock's face, or the overflow strip's own header, and the second reading
  places every bubble exactly where the first did — rule 5's pass over the base rects in a fixed order, so
  two browsers rendering one fleet agree with nothing stored ([§ 3.1](#31-the-keys-and-why-they-are-the-only-ones)); *(c)* on the overflowing run every
  overflow desk's furniture box and bubble lie below the floor's extent and inside the strip's box, and no
  furniture of the floor reaches below the extent — one scalar per floor in each direction, which is the
  boundary the reference's check states; *(d)* every bubble's box is sized from the measurer's answer and the
  title at the bound is drawn truncated with a mark (rule 4).
- **RED — the crowded map:** stub a `desks` layer whose two slot rects intersect → *(a)* fails naming both
  seats. Watched, because the console refuses nothing for it ([§ 14](#14-open-questions-for-the-review-loop) item 28): this test is what tells a map
  author, and a gate that could not fail on the one map defect the write does not catch would be a decoration.
- **Second RED — the order-dependent bubble:** resolve each bubble against already-displaced neighbours →
  the second reading places a bubble elsewhere, and two browsers on one fleet draw two floors.
- **Third RED — the strip on the floor:** draw the overflow row on the floor's own last band, which is where
  the reference drew it until card#7965 → *(c)* fails on the first overflow desk.
- **Fourth RED — the fixed-width bubble:** size the bubble from a constant → the title at the bound is
  clipped with no mark, and *(d)* fails — a clipped title read as the whole title is a claim about the wire
  the wire did not make ([§ 5.1](#51-the-desk) rule 4).
- **Discriminating control:** the shipped default map with the four `fx-snapshot-4` seats passes every clause,
  so the gate is known to be able to say *nothing overlaps*; and *(a)* over that map is the one clause an
  author moves — adjacent slots in the shipped default touch or nearly touch, so a character drawn wider than
  its desk is the first thing this control would refuse.

### AT-D3-21 the camera moves the viewer and never the fleet

*[§ 4.5](#45-the-viewport-rule-and-the-capability-floor)'s three rules, mechanised. Two halves, gated at their own steps: the **floor half** at [Appendix B](#appendix-b--what-an-implementer-builds-from-this)
...
- **Build — the floor half:** replay `fx-snapshot-4` through the scene at a viewport the fixture states,
  1,280 × 800; wheel-zoom at a stated cursor point, drag by a stated offset, fit-floor; then deliver a
  `seat.delta`, a full snapshot, a resync and a `room.map`; then the same entry at 1,279 × 800 and at
  1,280 × 799, then grown back; then [AT-D3-20](#at-d3-20-seat-furniture-never-overlaps-and-the-overflow-row-stays-below-the-floor)'s overflowing run with fit-floor. **Reads:** **the harness**, the
...
- **GREEN — the floor half:** after the wheel-zoom the scene point that was under the cursor is still under
  ...
  per seat, no map — and on growing back draws the floor again at fit, with no row for that either.
...
- **Third RED — the scaled-down floor:** at 1,279 × 800 draw the floor scaled to fit → the route renders a
...
- **Build — the building half:** `fx-snapshot-4` under a layout of two floors, one of them labelled, entered
  at `/`; ride to the labelled floor, ride to the other, whole-building; then the same under
...
- **Fifth RED — the label in the link:** ride to `/floor/{label}` → a route that does not resolve, which is
  [§ 4.4](#44-routes-and-what-each-one-fetches)'s ⛔ and card#9273's reason.
- **Sixth RED — the desks on the plate:** draw the floor's desks on its plate → a count a viewer counts by
...
28. **⇢ Review — the room is drawn and the camera built at [Appendix B](#appendix-b--what-an-implementer-builds-from-this) rows 14–16, and three things stay open
    ...
    *(1)* **The console refuses no overlapping `desks` objects.** [§ 10.3](#103-the-floor-map)'s table refuses an object outside
    the grid and refuses nothing for two inside it that intersect, so an authored map can put two slots on
    one spot and the room draws desks over desks. Meanwhile [AT-D3-20](#at-d3-20-seat-furniture-never-overlaps-and-the-overflow-row-stays-below-the-floor) holds the shipped default and a stubbed
    map, and its first RED is exactly that map. What would close it: a refusal row in § 10.3's table — a
    change to what the console accepts, and so the maintainer's or the operator's to rule; the
    recommendation is to add it, because a refusal at the write reaches the author before a viewer does,
    which is § 4.6's own argument for every other refusal in that table.
