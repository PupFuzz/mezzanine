...
the cross-section PR #96 built. Row 15's slice B (2026-09-26) built the camera and the capability floor:
...
the page only paints. The camera is one machinery at two scales, and the second scale is reached by a
...
- **`held`** — a render **held** for exactly as long as a delivered field has a value: the working,
  ...
  that ended the hold. Where no object ended it — [§ 2.3](#23-membership-a-seat-or-an-install-the-client-does-not-hold)
  row 4's backstop, and row 5, where the client stops vouching for a seat whose held object still says
  to draw it — [§ 11](#11-acceptance-tests) owns what the exit records, and why row 5's takes no row of
  this table.
...
**A hold ended by no object: [§ 2.3](#23-membership-a-seat-or-an-install-the-client-does-not-hold)
row 5's exit. This paragraph owns the rule, and [§ 6.2](#62-the-animation-table--the-closed-set)'s
`held` class points here** (card#7341, [§ 14](#14-open-questions-for-the-review-loop) item 29). When a
failed read takes a held seat's run of consecutive refused reads to row 5's threshold, the render that
follows draws that desk as the empty chair, and the held render open on it is left with an ordinary
`left` row of its own episode — `animation_id`, `install_id` and `seat_id` copied from its `entered`
row as every exit's are, `motion: false`, `at` the instant of that render — whose `cause` is the literal
**`unconfirmed`**. The rest of the rule follows from that:

- **It is no exit of a new kind, and it takes no row in [§ 6.2](#62-the-animation-table--the-closed-set)'s
  closed set.** Neither class admits one. An `edge` row needs a wire message the client applied, and a
  refused read is a response it could not apply; a `held` row needs a delivered field holding it, and
  the chair row 5 draws is held by the client's own count of refused reads, which the wire never
  delivered. So the chair enters nothing, exactly as a `stale` or `offline` chair enters nothing, and the
  exit is the one row the change writes. [§ 6.5](#65-a-snapshot-never-animates)'s reasoning lands on the
  same answer from the other side: the desk emptying is a claim about what the client knows, not about
  anything the seat did, so nothing about it moves.
- **The literal names what the client observed, and not one read of it.** What the client saw was
  refused reads, and no single one of them ended the hold — the first leaves the desk drawn, which is
  the threshold's whole purpose. What ended it is the threshold being reached, row 5's condition, and
  the desk model already carries that condition under this name. It is row 4's `snapshot` one case
  over: a literal wherever something the client observed ended a hold and no object did.
- **Only where no object ended it.** The render that draws the chair can also carry an applied object
  — a failed read's buffer is drained on the spot against what is held, applying any `+1` chain it
  holds ([§ 2.2](#22-connect-snapshot-deltas)) — and if the object the client then holds already makes
  the hold condition false, that object ended the hold and the row carries its `state_version`, as
  every other exit does. `unconfirmed` is written only when the held object still satisfies the
  condition the episode was entered under.
- **Never the held object's version, and never `null`.** That object still says to draw the render, so
  ...
  [AT-D3-1](#at-d3-1-no-animation-without-its-event)'s opposite-direction predicate exists to catch.
  `null` is this log's mark of a render with nothing behind it, and a correct exit written with the
  defect's value makes the defect unreadable.
- **Re-entry is ordinary.** A read that succeeds clears the count (row 5), the desk draws its character
  again, and the render it then holds is entered as a new episode against the fetched object's
  `state_version`. The unconfirmed stretch is the gap between two episodes, so *for how long* stays
  answerable on both sides of it.

**What [AT-D3-1](#at-d3-1-no-animation-without-its-event) holds such a row to**, in place of the object
predicate it cannot have: `unconfirmed` appears only as a `left` row's `cause`; the render that wrote
the row drew that seat's desk unconfirmed; and the object the client held for the seat at that render
still satisfies the hold condition of the episode the row closes — so the exit is attributable to the
refused reads, and to nothing the wire said.
...
| `fx-confirm` | ✅ landed with step 3 (`server/tests/Feature/Floor/fixtures/fx-confirm.json`, replayed by `Tests\Feature\Floor\TheClientProtocolReplaysTheSameWayEveryTimeTest`) and declared here since PR #227's design round: [§ 2.3](#23-membership-a-seat-or-an-install-the-client-does-not-hold) row 5's runs, each a seat the client holds whose confirming read is refused. **`missing_persistent` is the unconfirmed desk's run** — a snapshot, then every read of one held seat refused past row 5's threshold of refused reads, the stream carrying on — the one run in which the desk model's `unconfirmed` is true, which no snapshot can make it. The held render that exits there is ended by a failed read and not by an object, and [§ 11](#11-acceptance-tests)'s rule for a hold ended by no object is what that exit records ([§ 14](#14-open-questions-for-the-review-loop) item 29, closed). ⛔ [AT-D3-1](#at-d3-1-no-animation-without-its-event) does not replay the run yet: the client writes that exit against the held object's own `state_version` rather than the rule's literal, so the replay waits on [Appendix B](#appendix-b--what-an-implementer-builds-from-this) row 15's fix, and no test is loosened to admit it. A fixture sets values; it renders none, so this row is **`named-not-rendered`** |
...
- **Build — the closed-set half:** replay `fx-snapshot-4`, `fx-clear-trace`, `fx-degraded` and
  `fx-interns` end to end; collect the animation log. ⚠ Not yet `fx-confirm`'s `missing_persistent` run:
  the held render that exits there is ended by a failed read and not by an object, and
  [§ 11](#11-acceptance-tests)'s rule for a hold ended by no object says what that exit records and
  what this test holds it to. The run joins this half when the client writes that exit, which is
  [Appendix B](#appendix-b--what-an-implementer-builds-from-this) row 15's to deliver (the
  [fixture table](#11-acceptance-tests)'s `fx-confirm` row states where the client stands); until
  then it is left out, and no assertion here is loosened to admit it. **Reads:** the **animation log**, **the harness**, the
  **animation set**.
- **GREEN — the closed-set half:** every `animation_id` in the log is a row of
  ...
  — save a `left` row whose `cause` is the literal `unconfirmed`, which is held instead to the
  condition [§ 11](#11-acceptance-tests) states for a hold ended by no object — and the two phases are asserted in **opposite** directions, which is what makes both satisfiable on a
...
29. **✅ CLOSED — a failed-read exit is an ordinary `left` row of the episode it ends, and its `cause`
    is the literal `unconfirmed`; it is no exit of a new kind and takes no row in
    [§ 6.2](#62-the-animation-table--the-closed-set).** Doc-owner ruling, card#7341. Found at PR #227's
    round 4 (card#7341's rows 14–16 design round): [§ 2.3](#23-membership-a-seat-or-an-install-the-client-does-not-hold)
    row 5 makes a held seat the client cannot confirm render the empty chair, so a `held` render open on
    that desk exits; [§ 6.2](#62-the-animation-table--the-closed-set) recorded an exit against the object
    that ended the hold, and [§ 11](#11-acceptance-tests) required a `held` row's `cause` to be a
    `state_version` the client holds and a `left` row's object to no longer satisfy the hold condition —
    but a failed read advances no version and changes no object, so
    [AT-D3-1](#at-d3-1-no-animation-without-its-event)'s exit assertion would red on a correct client.
    ...
    **The rule is [§ 11](#11-acceptance-tests)'s**, in the paragraph under the animation-log schema
    ...
    restating it: the exit names row 5's threshold being reached, which is what the client observed and
    what ended the hold, and it is written only when no object applied in the same render already ended
    it. The same paragraph states what AT-D3-1 holds such a row to in place of the object predicate, and
    AT-D3-1's GREEN admits the literal on that condition alone.

    ⚠ **The document is answered and the client is not yet.** Where the shipped client stands on that
    run is [§ 11](#11-acceptance-tests)'s fixture table's to state, in its `fx-confirm` row; the client
    fix and AT-D3-1's replay of the run are
    [Appendix B](#appendix-b--what-an-implementer-builds-from-this) row 15's, the owner this item always
    named. **Reopens:** a second way for a held render to end with no object behind it — a client-side
    condition other than row 5's that stops a desk drawing what its held object says.
