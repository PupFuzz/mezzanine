...
| 6 | **Sort orders** | floors by floor id ascending, and a floor's rooms by `install_id` ascending ([§ 4.6](#46-the-building-layout)) — which on a floor with no plan is also the order they are drawn in, left to right (card#9292); desks by slot; timeline as served | Deterministic ordering of received objects |
...
edges aligned, **64 px** apart ([§ 12](#12-every-number-and-where-it-comes-from)), and no hallway. The floor's extent is the
...
hallway's, which is what makes them the floor's and what keeps a map from painting a second one
([§ 6.3](#63-forbidden-forms-named-so-they-cannot-be-written-in-good-faith)'s vocabulary test). A floor of five offices has one clock over all five.
...
1. **Extent lives in the room, position lives on the floor, and nothing lives in both.** A room's
   ...
   clipped — the operator moves the neighbour first, which is one layout save.
...
never saw, and the operator repairs it with one save.
...
not built (card#9292):** the shipped reader refuses a member it does not know by name, so a room
record, an `at` or a `hallway` is refused by the reader that ships today, and the fixture's cases
still author the bare form. [Appendix B](#appendix-b--what-an-implementer-builds-from-this) step 11 — the slice that moves the layout into the
...
| Gap between rooms on a floor with no plan | **64 px** | **Chosen** — two rooms' own perimeter walls must read as two rooms rather than as one double wall, and this is the smallest gap at which they do at floor zoom in the ratified reference's proportions; not derived from a tile grid, because the vendored tileset is an image collection with no grid to derive it from. **What would re-derive it:** a measurement on a built floor of two adjacent rooms, the way the viewport floor's row waits on a rendered nameplate. A planned floor has no gap rule — its spacing is the plan's (card#9292) | [§ 4.6](#46-the-building-layout) |
