...
same reason a published floor link does. ⚠ Stated as the doc fact it is: **the `/floor` route is
[Appendix B](#appendix-b--what-an-implementer-builds-from-this) row 8's artifact, the floor page**
(card#7341 step 8's ruling — the route inside the `auth`+`mfa` middleware group, its view, and the
entry that constructs the client protocol with a real `EventSource`), and it is served from the
moment that step lands. Until then this is what the route must do when it is written rather than a
behaviour anything exercises.
...
[§ 4.4](#44-routes-and-what-each-one-fetches)'s floor route as a SERVED route: it is
[Appendix B](#appendix-b--what-an-implementer-builds-from-this) row 8's artifact, the floor page
(card#7341 step 8's ruling), because no page may construct the client protocol before that step's
stream recovery exists — so until step 8 lands a composed floor is a plate in the lobby and not yet a
screen a viewer can open. ⭐ **What
...
therefore built except for the page that mounts it, which is Appendix B row 8's floor page; and the office map a room's `office` form will select — card#9269, **re-scoped by card#9292
