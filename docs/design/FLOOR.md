...
> [D2](FLEET-STATE.md) must be able to build the lobby, the floor and the drill-down. What has been
> built from it is recorded row by row in [Appendix B](#appendix-b--what-an-implementer-builds-from-this)'s landing markers. Every number carries its derivation; every failure
...
decision. [Appendix B](#appendix-b--what-an-implementer-builds-from-this) row 14 paints the room on it, and below the drawing each desk is its row of [§ 4.5](#45-the-viewport-rule-and-the-capability-floor)'s list view — a headless model over every fact the desk model emits, built at row 15's slice A. Until row 15's camera lands (slice B), the page paints that list at every viewport under the drawing; slice B makes it the render below the viewport floor. ⭐ **The drill-down form, `/floor/{floor}/{seat_id}`, is served** — [Appendix
...
under no camera (row 14's slice A) with the desks as text below it, at every viewport size. Row 15's
slice A has replaced row 8's page-side text render with **the list view** the first rule names — a
headless model over every fact the desk model emits, which the page only paints — and slice B, with
the camera, makes it the below-the-floor render. The camera is one machinery at two scales, and the second scale is reached by a
...
why the client needs no second implementation of the normalisation; and the floor screen ([§ 4.2](#42-the-floor)),
built at [Appendix B](#appendix-b--what-an-implementer-builds-from-this) rows 7 and 8, reads **this** rule rather than a third statement of it. A
...
plate, the stop and the floor screen — would have to recompute for itself;
...
29. **⇢ Review — an exit caused by a failed read has no object that ended the hold, and this document
    ...
    `missing_persistent`, [§ 11](#11-acceptance-tests)'s fixture table), AT-D3-1 does not replay it, and no test is loosened. ⚠ Row 15's list-view guard (slice A, `Tests\Feature\Floor\TheListViewRendersEveryDeskMemberTest`)
    now replays that run through the desk render and reads only the desk models it draws, never the animation log,
    so it asserts nothing about this exit's `cause`: the question stays open, and it is still owed before any test
    asserts on the log over that run.
