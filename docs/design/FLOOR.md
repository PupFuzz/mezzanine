...
- **What a test reads** is declared by the test, in a **`Reads:`** clause on each of its `Build`
  ...
  instrument Appendix B builds — a gate, which a test runs rather than reads. Any other bullet reds,
  because what runs it is undeclared, and a backticked name beginning `fx` that the table does not
  declare reds as a control.
