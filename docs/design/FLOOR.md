...
stopped taking messages. (4) **It does not surface a session that expires inside it**, for as long as
it lasts: F7's ordinary case — a floor left open until it reaches `SESSION_LIFETIME` — is detected by
neither channel while the grace holds. An `EventSource` open failure carries **no status code**, so the
`401` every re-open now earns is indistinguishable from the `503` the grace was written to ride out;
and clause (2)'s *no poll is made* removes the REST `401` that
...
the grace, so this is unreachable by construction and not by oversight. The viewer sees the floor as it
stood — *live*, then *reconnecting* — rather than F6's sign-in prompt, for the remainder of the grace,
at most the 60 s above. **Accepted:** the only thing that would tell the two refusals apart inside the
grace is the poll the grace exists to suppress, whose render is the *feed down* over a routine deploy
that ruling A4 declined; and the delay is self-clearing, because the grace's end is step 7, whose first
poll returns that `401` and fires F6. **What it costs, stated:** the 60 s is computed at this
repository's defaults, and a host that raises `MEZZ_FEED_DRAIN_CEILING_S`, or sets an
`opcache.revalidate_freq` whose wait outlasts the drain, can hold the window past it. No client can
read either; such a deploy's viewer sees step 7's render for the remainder — loud and wrong about the
cause, never a floor claiming health.
...
- **GREEN:** `503` renders the store-unavailable statement — on a warm client over a floor labelled
  ...
  *a held-open stream rendered as a live one* — so a build passing this gate shipped the posture the
  ...
  [§ 9](#9-failure-paths-and-their-observables) F6 unreachable inside the grace**, which
  [§ 2.2](#22-connect-snapshot-deltas)'s clause (4) states as an accepted cost — so no leg here asserts
  a sign-in prompt for a session that expires inside the grace, and one added later would be asserting
  against that clause rather than finding a bug. A stream that ends with **no** `feed.close`
  — the drain's — takes F1's render at once, with no grace.
