...
| **Feed backpressure** | a client cannot drain the stream | **CLOSED for that connection** — a tick the handler cannot complete within **45 s** is detected at the top of the following pass and ends the stream with `feed.close`, and other clients are untouched ([§ 8.5](#85-gaps-reconnect-and-why-state_version-is-not-seq), whose bound is a detector and not a cap on how long the worker is held) | Dropping deltas silently leaves that browser permanently and invisibly wrong. Ending the stream costs one snapshot fetch and is self-healing — and returns the worker, which under SSE is the resource a stalled client actually consumes. |
...
| `reporter.selftest_failed` | array\<string\> | no | **0…8**, the failing check names. **Not the member count of D1's table on the day this row was read:** [D1 § 6.14](EVENT-SCHEMA.md#614-reporterheartbeat) declares that key set **open at the ingest**, so a conforming reporter may ship a check that table does not yet name and take no `422`, and a consumer validating against the table's current membership would reject valid data. **8 is D1's own ceiling**, not a count copied from it — the per-key bound and the 256 B serialized cap leave room for no ninth member, whatever the table holds — and D1's arithmetic is where it is derived. ⚠ A prose quotation of D1's ordinals stood here and drifted the moment D1 gained a member (`card#9296`); the bound survived and the sentence around it did not, which is why the bound is now stated against the CAP rather than against a count. The names total ≤ 175 B across 8, which is what D1's 256 B serialized cap on `selftest` leaves | `[]` |
...
- **It is a label, not a member of the binding — and it carries a uniqueness INVARIANT no schema
  ...
  binding, and they are the whole of it"* is unchanged: nothing resolves a seat by this member and no
  `UNIQUE KEY` covers it. ⭐ **But a protocol agent name is UNIQUE among the seats of one
  ...
  either of them. ⛔ **No seat can detect it, which is why the invariant is stated here and enforced
  at the consumer.** A seat's own check asks *is my declared name in the roster*
...
instrument, the baseline-then-pool-override rule and the *card#9300 owes the reading before it gates a
deploy* posture are all R1's. A third top-level condition would put one kind of fact in two homes and
...
`php-fpm -i` and then the serving pool's override, and card#9300 owes that reading. *The observable
...
reload can finish over a half-open stream whose client vanished without a `FIN`. ⛔ **And BOTH pools
set `pm.status_path`** — a requirement, not a convenience of the check below, because
[§ 2.1](#21-processes)'s `mezzanine:feed-reload` **polls the stream pool's active-process count** to
decide when the streams have gone before FPM is reloaded, and a pool without a status path answers
that poll with nothing. Deploy behaviour depends on it, so it is stated here with the pool's other
three values rather than as a caveat on an instrument (card#9287, maintainer round).
...
which R1 names as a third ini condition and hands to card#9300 with the other two. The worker comes back because the request was
...
file rather than the resolution. That gap is card#9300's, with R1's — the same rule applied to this
condition: an instrument is named after it has been run on a host where it can be, not before. *The observable
...
1. **The join is `(install_id, protocol_agent_name)` → the ONE seat of that install declaring that
   ...
   - **Nothing declared it** — no seat of this install carries the name with a resolving check
     state. That is the empty case, it is `card#7957` ruling *(2)*'s permanent first-class render,
     and it is what a fleet whose seats declare nothing produces everywhere.
   - ⛔ **More than one seat declared it** — the invariant is **violated**, and the name resolves to
...
`ignore_user_abort` is **off** on the serving SAPI, a host condition R1 now names and card#9300 owes
the reading of. The conclusion stands; the reason it stands is a setting, not the framework. The stall bound is the SLOW consumer's exit
...
- **(a) Every pass inside [§ 8.5](#85-gaps-reconnect-and-why-state_version-is-not-seq)'s 45 s stall
  ...
  **under 60 s** — and on a client that drains promptly it is **15 s + one 250 ms tick**, which is the
  figure a test on a healthy consumer asserts.
- **(b) A pass that OVERRUNS the stall bound.** ⛔ **That bound is a DETECTOR, not a cap**: it is
  compared at the top of the FOLLOWING pass, so a write that blocks for longer than 45 s does not end
  at 45 s — it simply makes that pass the last one, and the stream then ends on `stalled` rather than
...
| 50 | **A protocol agent name is UNIQUE per install; a name two seats declare is a misconfiguration that resolves to nothing and is reported as its own reason** ([§ 8.2.1](#821-the-seat-state-object), [§ 8.3.3](#833-the-coordination-objects), [FLOOR.md § 5.7](FLOOR.md#57-the-coordination-thread-line)) | ⭐ **Operator ruling, 2026-09-12, `card#9296`.** The alternative on the table was the one three surfaces of this document had already written down: a name may resolve to **a set**, because one agent running two seats is two desks and not a collision | The set reading legalised a resolution nothing could then govern. [FLOOR.md § 6.2](FLOOR.md#62-the-animation-table--the-closed-set) A18 triggers on participants resolving to *a desk*, singular, and the client's `resolve()` returns one `seat_id` — so the map a builder was told to construct was last-writer-wins over an unordered seat population and A18 drew to whichever seat was folded last. That is the guessed desk § 5.7 clause 1 forbids, arrived at by obeying the documents. Ruling the duplicate a **misconfiguration** removes the pick instead of specifying it | **a seat cannot enforce it.** Its own check asks whether its name is in the roster, and two seats declaring one name both pass — so the invariant is declared on this plane and enforced at the consumer, which is the only party that sees both seats. Until a fleet actually declares, the arm is unreachable in practice and rests on review rather than on a red |
