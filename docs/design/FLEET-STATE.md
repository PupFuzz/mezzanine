...
and not a fleet event; fetch the layout again on a `building.layout`; and on a map request that fails,
draw the failure by name rather than the shipped default in its place, because a default drawn
silently is a room the operator authored rendering as one they did not.
