...
GET /api/fleet/stream                       -- session + MFA middleware (§ 9); text/event-stream
  ...
  loop:
     ...
     if now - auth_done >= 15 s:
         if not session_and_mfa_still_valid(reader):        -- re-READ from the store (§ 9)
...
failed session re-check, or when a write fails because the client is gone. *What this repository
...
middleware ran at connect — the session is live and its MFA enrolment holds — and a failed check ends
the stream with `feed.close{reason:"session"}`**. ⛔ **It re-READS both from the store**, the session
