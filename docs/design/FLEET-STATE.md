...
| **feed heartbeat** | long-lived daemon (`mezzanine:feed-heartbeat`), supervised | every **15 s** | write [§ 8.3](#83-the-websocket-delta-feed)'s `feed.heartbeat` to `feed_outbox` — **one row, fleet-wide, unconditionally** — whether or not anything changed and whether or not a client is connected — and, on a tick where `db`, `fold` or `sweep` changed value, [§ 8.3](#83-the-websocket-delta-feed)'s `fleet.health` for that change. A read of the store that fails publishes `db: "down"` rather than exiting — ⛔ **but only where the store is readable-but-degraded, and an earlier revision of this row overstated it.** This daemon's publish target is `feed_outbox`, which is in the SAME store ([§ 6.4](#64-ddl)): in a full outage it can write nothing, so it is not the messenger of that outage and this row no longer claims to be. Who tells whom, exactly: a **connecting** browser is told by the handler's on-connect `fleet.health`, a direct yield needing no outbox row ([§ 2.2](#22-fail-posture-per-path)'s stream-connect row); an **already-connected** browser is told by this daemon where the store still takes writes, and otherwise by the heartbeat's ABSENCE, which arms its 45 s dead-feed timer ([§ 8.3](#83-the-websocket-delta-feed)) — the one case where silence is the message, and the reason the heartbeat is unconditional | **a quiet fleet and a dead stream stop being distinguishable — the one thing [§ 8.3](#83-the-websocket-delta-feed) built this message to separate.** The client's 45 s timer is armed by a message of *any* kind, so a stream with `seat.delta` traffic stays up by accident and a **quiet** fleet's stream — precisely the case the heartbeat exists for — renders `feed_down` and reconnect-loops against a perfectly healthy fleet. A `db`/`fold`/`sweep` change is also never announced to a connected client, this daemon being that message's producer. Nothing errors: receipts land, the snapshot serves, the deploy is green |
| **feed stream** | HTTP request (PHP-FPM), **long-lived** — `GET /api/fleet/stream`, one per open browser, and a worker pinned for as long as it is open ([§ 8.3](#83-the-websocket-delta-feed)) | polls `feed_outbox` every **250 ms**; re-checks the session every **15 s** ([§ 9](#9-read-side-authentication)) | yield `fleet.health` first — `db: "down"` if the store is unreadable ([§ 2.2](#22-fail-posture-per-path)) — then deliver every outbox row past its cursor that [§ 9](#9-read-side-authentication)'s filter admits, in `id` order, behind [§ 6.5](#65-the-fold)'s 2 s visibility lag; end the stream on `fleet.reload`, on [§ 8.5](#85-gaps-reconnect-and-why-state_version-is-not-seq)'s stall bound, or on a session re-check that came back **invalid**, saying which with `feed.close` — never on one that could not REACH the store, which is [§ 9](#9-read-side-authentication)'s third outcome and ends nothing | that browser's `EventSource` errors and the client re-opens on its 10 s cadence ([FLOOR.md § 9](FLOOR.md#9-failure-paths-and-their-observables) F1, F3) — every other stream is a separate worker and notices nothing. **What a dead pool looks like is the one failure here that is not the feed's:** with every worker pinned, the snapshot, the health endpoint and the console queue behind the streams, which is § 8.3's requirement R2 and its named observable |
| **feed reload** | deploy command (`mezzanine:feed-reload`), run by `bin/deploy.sh` **before** PHP-FPM is reloaded | per deploy | write one `fleet.reload` row to `feed_outbox` carrying the release's `feed_version`, **then wait for the streams to go**: a row is invisible to every handler for [§ 6.5](#65-the-fold)'s 2 s visibility lag and is delivered on the tick after that, so the command returns no sooner than **lag + tick + margin (3 s)**, and then polls the stream pool's active-process count until it reaches zero **or a 30 s ceiling elapses, whichever comes first — at the ceiling it returns success and logs the residual count**, because a deploy must not block indefinitely on one wedged stream and FPM's own reload escalation is the backstop for whatever is left (R2's `process_control_timeout`). ⚠ *Preferably* stood here in an earlier revision, which is not a buildable instruction: one builder writes `sleep 3`, another blocks forever, and the two differ on whether a single frozen client can stall every deploy. Every open stream then delivers it and **ends** ([§ 8.3](#83-the-websocket-delta-feed)), which is what returns the pinned workers before the reload asks for them — ⚠ a deploy that writes the row and reloads in the next line reloads while every stream is still inside the lag window, returns no worker by this path, and falls back on FPM's own escalation, whose behaviour at the default `process_control_timeout` this document marks UNVERIFIED (R2) | streams keep serving the previous release's code until FPM's own reload escalation finishes them (R2's `process_control_timeout`), and each client learns of the deploy from the first envelope whose `feed_version` it does not know ([§ 8.1](#81-two-surfaces-two-compatibility-postures)) rather than from a message that says so — later, and by the failure path instead of the announcement |
...
| **Stream connect** | the app up, the store down | **CLOSED** — the request is accepted and the handler's **first message** is `fleet.health` with `db: "down"`; no snapshot is served | Same argument as the REST read. The stream stays open deliberately, because it is the channel that tells the browser *why* there is nothing — and under SSE the on-connect message is the handler's first yield rather than a hook nobody had ([§ 8.3](#83-the-websocket-delta-feed)). |
...
| **Feed backpressure** | a client cannot drain the stream | **CLOSED for that connection** — a tick the handler cannot complete within **45 s** ends the stream with `feed.close`, and other clients are untouched ([§ 8.5](#85-gaps-reconnect-and-why-state_version-is-not-seq)) | Dropping deltas silently leaves that browser permanently and invisibly wrong. Ending the stream costs one snapshot fetch and is self-healing — and returns the worker, which under SSE is the resource a stalled client actually consumes. |
| **Feed stream, ALREADY OPEN** | the store becomes unreachable mid-stream | **OPEN, and the stream becomes the messenger** — the tick's read fails, the handler yields `fleet.health{db:"down"}` (at most once per outage, not once per tick), does **not** advance the cursor and does **not** end the stream; delivery resumes on the first read that succeeds, with `fleet.health{db:"up"}` on that tick ([§ 8.3](#83-the-websocket-delta-feed)) | Symmetrical with the stream-connect row above, and for a sharper reason: ending these streams would put **every browser on the fleet** onto the 10 s reconnect cadence for the length of the outage, against a pool R2 sizes in concurrently open streams — so a store outage would take the console down with it by [FLOOR.md § 9](FLOOR.md#9-failure-paths-and-their-observables) F20, which is the collateral this transport exists to make impossible. A held-open stream costs one pinned worker that was already pinned |
...
CREATE TABLE feed_outbox (
  id          BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
  created_at  DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),  -- the store's clock, stamped inside the
                                                                  -- writer's transaction: § 6.5's visibility
                                                                  -- lag and § 6.7's purge both read it
...
| `feed_outbox` | **60 s** after `created_at` | **the only transient table in this store, and the only one whose retention is measured in seconds.** It is [§ 8.5](#85-gaps-reconnect-and-why-state_version-is-not-seq)'s stall bound (45 s) plus one heartbeat interval (15 s): a stream that resumes at the edge of its bound finds every row it may still deliver, and a stream past it has already been ended and never reads again — which is a property of WHERE the bound is checked, not only of its value: [§ 8.3](#83-the-websocket-delta-feed)'s handler tests it **before** its read, so a stream that was blocked longer than this retention ends instead of advancing its cursor over a row the purge took. Were the test after the read, this row's number would have to bound something no clock in the handler measures. It is **not** a replay buffer — no stream starts below the head it connected at ([§ 8.3](#83-the-websocket-delta-feed)) — so no reader ever needs a row older than the longest a reader may lag, and 60 s is that figure with the margin stated. **A row nobody consumed is the ordinary case** — no browser open, and the writers write regardless, as the heartbeat daemon always did — **and it is purged unread**: the fact it announced lives in `seat_state` and the projections and reaches a later reader at its snapshot. The one message class no store holds, the coordination objects, is lost to an absent reader exactly as § 8.5 already says it is lost to a dropped one |
...
everything, and it is where a per-install ACL attaches when [§ 14](#14-open-questions-for-the-review-loop)
item 7 is ruled. The earlier revision's `private-fleet.{install_id}` channel is retired with the
...
| `fleet.reload` | server → client | a deploy — written by `mezzanine:feed-reload` ([§ 2.1](#21-processes)) before PHP-FPM is reloaded, carrying the release's `feed_version` whether or not it changed, because the deploy is the one act that knows the code moved. ⛔ **Terminal**: the handler ends the stream after delivering it, which is what returns a pinned worker before the reload asks for it. ⚠ **The message is written on every deploy; the client's RELOAD BANNER is not raised by every one** — it is raised on a `feed_version` the client does not know ([§ 8.1](#81-two-surfaces-two-compatibility-postures), [FLOOR.md § 9](FLOOR.md#9-failure-paths-and-their-observables) F8), which is unchanged by this row. An unchanged `feed_version` after a deploy means **the wire did not move**, and a client whose bundle is one release old is by that definition still correct on it — so it reconnects, keeps rendering, and is asked to reload only when the wire actually changes. Reading the terminal end as a reload prompt would put a banner in front of every viewer on every deploy | `feed_version`, `reason` |
...
| `feed.close` | server → client | the handler is ending this stream **on its own decision**, and says why, and the set is **exactly three**: `stalled` — a tick did not complete within [§ 8.5](#85-gaps-reconnect-and-why-state_version-is-not-seq)'s 45 s bound; `session` — [§ 9](#9-read-side-authentication)'s 15 s re-check **reached the store, and the store said the session or its MFA is gone**. ⛔ A re-check that could not REACH the store is § 9's *unverifiable* outcome, which is not a verdict about the session: it ends nothing and appears nowhere in this set, exactly as a failed **tick** read does; `unavailable` — the **connect** read of `feed_outbox` failed, so this stream never had a cursor to start from (a **tick** read that fails does not end the stream and does not appear here — the stream stays open as the messenger of the outage, [§ 2.2](#22-fail-posture-per-path)). `unavailable` is treated by the client exactly as an ordinary drop: reconnect on the 10 s cadence. A stream that ends with no `feed.close` ended for a reason the server did not choose (a worker killed, the proxy, the network), and the client treats the two alike except that `session` sends it to sign in rather than to reconnect ([FLOOR.md § 9](FLOOR.md#9-failure-paths-and-their-observables) F3, F7) | `reason`, `at` |
...
the stream stays open in that posture. It is a separate message type from `feed.heartbeat` even though
...
response **silently**, with no `feed.close`, after whatever had already flushed. A store outage would
have put every browser on the fleet into a 10-second reconnect loop for the length of the outage,
against a pool R2 sizes in concurrently open streams — the outage taking out the console as collateral,
by F20, which is the one failure this transport is supposed to make structurally impossible. The two
reads fail differently, and the difference is deliberate: the **connect** read has no cursor to fall
back on, so it ends the stream; a **tick** read has one, so the stream stays open and becomes the
messenger of the outage, which is the same argument [§ 2.2](#22-fail-posture-per-path) already makes for
connect:
...
GET /api/fleet/stream                       -- session + MFA middleware (§ 9); text/event-stream
  ...
  cursor    = SELECT COALESCE(MAX(id), 0) FROM feed_outbox   -- ON FAILURE: cursor = 0 is REFUSED
               WHERE created_at <= server_now - INTERVAL 2 SECOND   -- (it would replay the whole
                                            ...
                                            -- reconnects on its own 10 s cadence.  THE HEAD BEHIND
                                            -- THE LAG, not
  ...
  loop:
     ...
     rows = SELECT id, t, install_id, message FROM feed_outbox   -- ON FAILURE: yield
             WHERE id > cursor AND created_at <= server_now - INTERVAL 2 SECOND   -- fleet.health
             ORDER BY id                                  -- {db:"down"} (at most once per outage,
                                            -- not once per tick), DO NOT advance cursor, DO NOT end
                                            -- the stream; resume delivery when a later read succeeds
                                            -- and yield fleet.health{db:"up"} on the recovery tick
     for row in rows:
         if visible_to(reader, row): yield row.message     -- § 9's per-subscriber filter
         cursor = row.id
         if row.t == "fleet.reload": return                 -- terminal (§ 2.1's feed reload)
     if now - auth_done >= 15 s:
         switch session_and_mfa_recheck(reader):            -- re-READ from the store (§ 9), and it
             case valid:        pass                        -- has THREE outcomes, not two
             case invalid:      yield feed.close{reason:"session"}   -- the store ANSWERED: gone
                                return
             case unverifiable: yield fleet.health{db:"down"}        -- the store could not be REACHED:
                                            -- nothing was learned about the session, so DO NOT close
                                            -- (§ 2.2's already-open row).  same at-most-once-per-outage
                                            -- suppression as the tick read above, and the cursor is
                                            -- untouched here as it is there
         auth_done = now                    -- re-stamped on ALL THREE outcomes: the auth interval is a
                                            -- clock, not a result, so an outage costs one re-check
                                            -- attempt per 15 s and not one per 250 ms tick
```
...
belong in `bin/deploy.sh` as a gate that refuses on a red. R1's **wire** half needs a stream opened
...
every stream on a schedule. ⛔ **The two ini settings are NOT one hazard, and an earlier revision of this section put them in one
...
the branch a non-zero `output_buffering` opens, so the handler may well defeat it. **Which is true on a
real FPM host is UNMEASURED, and this document does not assert it either way.** That is not academic —
the box this design was written on carries `output_buffering = 4096` in its *FPM* ini, so if the flush
does not defeat it, that host is R1-false today and every browser on it renders F19. Card#9300 owes the
measurement — serve the stream from FPM with `output_buffering = 4096` once and look — before either
value is written into a gate that refuses a deploy.
...
cannot see:* the pool's `pm.max_children`, `request_terminate_timeout` and `process_control_timeout`.
...
renders as every browser reconnecting in lockstep) and `process_control_timeout` is **non-zero**, so a
reload can finish over a half-open stream whose client vanished without a `FIN`.
...
the primitive breaks out of its `foreach` on the aborted connection, and `break` abandons a **suspended
generator** — no line after the pending `yield` ever runs. There is no `feed.close`, no
`feed_resync_required`, and no cursor left to inspect. The worker comes back because the request was
...
arriving from clients rather than from load. *The check:* read the effective pool configuration (`php-fpm -tt`, or the pool file) for the three
values and confirm the stream route resolves to the dedicated pool; then open N streams with the R1
command in N shells and, with all N open, `GET /api/fleet/health` — which is served by **the other**
pool, and that is the point of the check — must answer at its ordinary latency, while the **stream
pool's** `pm.status_path` shows `listen queue` at 0 and `max children reached` unmoved. ⚠ Both pools
need `pm.status_path` set for this to be readable at all; a pool without one answers the check with
nothing, which is not a pass. *The observable
...
rather than one feature is why this is a requirement with a check and not a note. ⚠ Whether a graceful
reload with `process_control_timeout` at its default of 0 waits on an in-flight stream indefinitely is
**UNVERIFIED** here (DOCS-CITED, `php-fpm.conf`: the default is stated, the behaviour at it is not),
which is why the design relies on it in neither direction — live streams end themselves on
`fleet.reload`, and the setting is required non-zero for the dead ones.
...
**Backpressure — a stall bound of 45 s per stream, and why it is a time and not a count.** The
...
returned. Both placements are the bound rather than details of it, and
...
the purge took while it was blocked, which is the silent loss the last paragraph here refuses. 45 s is the client's own dead-feed
...
untouched, because each is its own worker. What a stalled client *does* consume is that worker, for up
to the bound — which is R2's arithmetic ([§ 8.3](#83-the-websocket-delta-feed)), and the reason the
bound is not longer.
...
get a final pass** — the primitive `break`s out of its loop on the aborted connection, and that
abandons a suspended generator, so no line after the pending `yield` runs: no `feed.close`, and
`feed_resync_required` is **not** counted on this path. The stall bound is the SLOW consumer's exit
...
until the kernel gives up on the connection, which is R2's `process_control_timeout` clause and its
teardown clause, named there rather than here.
...
it
([§ 8.3](#83-the-websocket-delta-feed)). The lag between an expiry and its enforcement is therefore, **on a re-check that can reach the store**, at
...
⛔ **The re-check has THREE outcomes and not two — valid, invalid, and *unverifiable* — and
*unverifiable* is NOT a verdict about the session.**

- **valid** — the store answered, the session is live and its MFA enrolment holds: the stream continues.
- **invalid** — **the store answered, and what it said is that the session or its MFA is gone**: the
  stream ends with `feed.close{reason:"session"}` ([§ 8.3](#83-the-websocket-delta-feed)). That is the
  revocation-on-an-already-open-connection case this re-check was written for, and its meaning is
  exactly this one.
- **unverifiable** — **the check could not REACH the store, so it learned nothing about the session.**
  The stream takes [§ 2.2](#22-fail-posture-per-path)'s *feed stream, ALREADY OPEN* path and **stays
  open**: `fleet.health{db:"down"}` is the observable — the same message the tick's failed read yields,
  at most once per outage and not once per tick — the cursor does not advance, and there is **no
  `feed.close` of any reason**. ⛔ **The closed set of three `feed.close` reasons is unchanged and
...
15 s, so on a store outage the re-check always runs first. A re-check with two outcomes lands a failed
READ on *invalid* and ends every open stream within 15 s of the store going down —
`feed.close{reason:"session"}`, which sends the client to sign in rather than to reconnect
([FLOOR.md § 9](FLOOR.md#9-failure-paths-and-their-observables) F6/F7) — onto a sign-in page that
**cannot be completed, because authenticating reads the same tables the check just failed on**. It also
renders *your session is gone* when what is true is *the store is down*, which is the absence of
evidence read as evidence of absence, on the surface a viewer's authentication is decided from. The
render this posture owns is F5's, at
[FLOOR.md § 9](FLOOR.md#9-failure-paths-and-their-observables) F21.

**The one posture this does NOT contradict is [§ 2.2](#22-fail-posture-per-path)'s *read-token
verification* row**, which fails CLOSED — *"there is no posture in which 'we could not check, so we
allowed it' is correct"* — and the difference is not a carve-out from it. That row is about **granting**
access at request time to a credential that gates the whole fleet's activity picture, and a **connect**
under this same outage is that case and is unchanged: the route's middleware checks against the store
like any other request, and a stream that cannot be authorized is not opened. This outcome is about
**revoking** access already granted, on a stream that in this exact posture is carrying **no fleet data
at all** — the cursor does not advance, no outbox row is delivered, and the only message the handler
emits is `fleet.health{db:"down"}`. A re-check with nothing to protect buys nothing by closing, and
spends the operator's one remaining instrument for watching the outage.

⚠ **The one case that argument does not cover in those words, named rather than left for a reader to
find: a session read that fails while reads of `feed_outbox` keep succeeding.** They are two tables in
one store and usually fail together, which is the posture above — but a lock, a corrupt page or a
per-table permission can take one and not the other, and then the stream **is** delivering while the
session cannot be verified. *Unverifiable* still holds it open, and what that costs is stated rather
than hidden: deltas continue to a session last confirmed valid one auth interval ago, for as long as
the session read keeps failing. Closing still buys nothing — the credential that would re-authenticate
is read from the very table that could not be read, so the sign-in cannot complete in this case either —
and the exposure ends on the first re-check that answers, in whichever direction it answers.

⚠ **What this leaves unresolvable is stated here rather than engineered around: while the store is
down, an EXPIRED session and an UNVERIFIABLE one are indistinguishable.** If an outage outlasts
`SESSION_LIFETIME` — **120 minutes** on this application's configuration, below — every open stream is
in that state, and nothing in the handler can tell which. **No timer, no heuristic and no client-side
guess is specified for it**, and none would be an improvement: each would be a verdict invented from an
absence of evidence, which is the defect this third outcome exists to remove, re-minted one layer down.
What bounds it is that the stream is delivering nothing while it lasts. **The first re-check after the
store answers again resolves it truthfully**: if the session really had expired, that check returns
*invalid* and the stream ends with `feed.close{reason:"session"}` then — late by the length of the
outage, and correct. So the enforcement lag stated above is the lag *while the store is readable*; under
an outage it is the outage plus one auth interval, and that is the accepted cost of not guessing.
...
| 9 | the stream handler and `feed_outbox` — the on-connect `fleet.health`, deltas in `id` order behind the visibility lag, the feed heartbeat, the stall bound, the session re-check, `mezzanine:feed-reload`; `bin/deploy.sh`'s edit (retire the Reverb unit, add `mezzanine:feed-reload` **before** the FPM reload, run R2's and R1's ini checks as a gate — R1's wire half is the operator runbook, [§ 8.3](#83-the-websocket-delta-feed)); and the retirement of the previous transport's wiring. ⛔ **The retirement's blast radius is larger than the three files an earlier revision of this row named, and two of this row's own gates run on what it deletes.** Re-derive the set rather than trusting a figure — `grep -rln -i broadcast --include=*.php server --exclude-dir=vendor --exclude-dir=node_modules` — and note that the grep is not the audit ([canon #7](../../CLAUDE.md)): `tests/Feature/Feed/FeedTestCase.php` installs a `CapturingBroadcaster`, and the classes that INHERIT it (`grep -rl 'extends FeedTestCase' server/tests` — re-derive) do not contain the search term. Named because each needs a decision, not just a deletion: `bootstrap/app.php`'s `->withBroadcasting(...)` is the call that registers `/broadcasting/auth` at all; `app/Events/SeatRetired.php` carries `ShouldBroadcastNow` **outside** `app/Feed`; `AuthSurfaceTest`, `MfaGateTest` and `IngestAuthSeparationTest` assert the MFA stack **on `/broadcasting/auth`**, and that assertion must MOVE to the stream route rather than be deleted with it — `routes/channels.php`'s own comment says it must not be removed, and it is the only thing standing between the fleet's activity picture and an unauthenticated reader. ⭐ **Every test that reaches the broadcaster must be re-pointed before it goes, and the set is stated here as a DERIVATION rather than as a list, because a list in this row was already wrong once**: re-point every file `grep -rln '$this->wire->' server/tests --include=*.php` returns, and expect every class `grep -rl 'extends FeedTestCase' server/tests` returns to fail at `setUp()` regardless — `FeedTestCase::setUp()` constructs the broadcaster for all of them, whether or not they touch it. ⛔ **Run both greps; do not build from a set written here.** The `-i broadcast` grep above does not reach them: a test can drive `$this->wire` and never spell the word, which is exactly how an earlier revision of this row came to name AT-D2-7 and AT-D2-8 alone and miss the rest. Under SSE these tests read `feed_outbox` rows and a consumed `text/event-stream`, not a captured broadcast; keeping `CapturingBroadcaster` alive to hold them green would leave them testing retired code — a green over a mechanism nobody reached, which is the posture this card exists to refuse. ⚠ **ORDERING — AT-D2-23 is in that set and step 11 gates on it**, so this row must not delete the broadcaster while a later step's gate still needs it: either re-point AT-D2-23's test HERE, so step 11 runs against the re-pointed one, or sequence the deletion after step 11. Step 8 gates on AT-D2-23 too and runs before this row, so that test has to be green on both sides of the deletion — which is the argument for re-pointing it here rather than re-sequencing | [AT-D2-7](#at-d2-7-snapshot-then-deltas-has-no-window), [AT-D2-8](#at-d2-8-a-delta-gap-is-detected-and-resynced), [AT-D2-15](#at-d2-15-feed-backpressure-closes-one-connection-and-no-others), [AT-D2-25](#at-d2-25-a-concurrent-writer-cannot-strand-a-message-behind-a-streams-cursor) |
...
| 11 | **retirement** — the three columns, the recomputed render, the `cause: operator` transition row and the two publishes, in one transaction, behind whichever operator entry points § 2.1 lists ([§ 2.1](#21-processes), [§ 4.10](#410-retirement-is-a-rendered-state)). It comes after step 9 because it publishes on the feed. ⚠ **And it therefore DEPENDS on what step 9 deletes**: its gate AT-D2-23 is one of the tests that drives step 9's `CapturingBroadcaster`, so step 9 must leave that test runnable — re-pointed, not deleted with the class — or this row has no gate left to run. Step 9 states the same hazard at the deletion | [AT-D2-23](#at-d2-23-a-retired-seats-desk-goes-and-only-an-announcement-removes-it) |
