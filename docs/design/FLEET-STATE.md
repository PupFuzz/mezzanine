...
| **Counter closure** | every counter WRITTEN — a counting verb before or after its name — in prose, an AT or a pseudocode fence against [§ 7.1](#71-d1s-server-side-counters--where-they-live) / [§ 7.2](#72-this-planes-own-counters-and-badges), the name backticked or, as the fences write it, bare snake_case (`count feed_resync_required`); ⚠ outside that population, and declared at the tool: a fence's `x += 1`, which no spelling separates from [§ 6.5](#65-the-fold)'s `state_version += 1`, and a bare name with no `_`; including the `Stored` and `Exposed` columns; that a counter naming **fleet health** as its surface is carried by [§ 8.2.4](#824-the-fleet-health-object)'s `counters` member, because a surface that does not carry the thing is no surface; D1 § 12.7's population against § 7.1; and the **count** of that population as this document states it in prose | **tool-checked** |
