...
| `batches_refused.<error>` | `seat_counters`, one row per error code — including `server_error`, the store-failure refusal [§ 2.2](#22-fail-posture-per-path) answers | seat detail | **none** — the reporter observes the same refusal from the other side and D1 § 9.3's `batches_rejected` member is raised by its own counter. A second badge for one condition would be a second home for one fact, and the two counts disagreeing is itself the signal |
