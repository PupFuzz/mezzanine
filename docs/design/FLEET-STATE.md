...
| **feed heartbeat** | long-lived daemon (`mezzanine:feed-heartbeat`), supervised | every **15 s** | write [§ 8.3](#83-the-websocket-delta-feed)'s `feed.heartbeat` to `feed_outbox` — **one row, fleet-wide, unconditionally** — whether or not anything changed and whether or not a client is connected — and, on a tick where `db`, `fold` or `sweep` changed value, [§ 8.3](#83-the-websocket-delta-feed)'s `fleet.health` for that change. A read of the store that fails publishes `db: "down"` rather than exiting: [§ 2.2](#22-fail-posture-per-path)'s stream-connect row makes the stream handler the messenger of the outage on connect, and this daemon its messenger for the change | **a quiet fleet and a dead stream stop being distinguishable — the one thing [§ 8.3](#83-the-websocket-delta-feed) built this message to separate.** The client's 45 s timer is armed by a message of *any* kind, so a stream with `seat.delta` traffic stays up by accident and a **quiet** fleet's stream — precisely the case the heartbeat exists for — renders `feed_down` and reconnect-loops against a perfectly healthy fleet. A `db`/`fold`/`sweep` change is also never announced to a connected client, this daemon being that message's producer. Nothing errors: receipts land, the snapshot serves, the deploy is green |
...
CREATE TABLE feed_outbox (
  ...
  t           ENUM('seat.delta','feed.heartbeat','seat.retired','fleet.reload','fleet.health',
                   'coord.thread','coord.round','room.map','building.layout','feed.close') NOT NULL,
...
ceiling is 8,980 × 50 ÷ 24 ≈ 18,700 rows (~7 MB at ~400 B a row — [§ 6.8](#68-sizing)) and is never read
...
| `feed_outbox`, lingering between purges | **~7 MB** | ≤ 1 h of ceiling traffic between hourly purges ([§ 6.7](#67-retention-and-purge)): 8,980 × 50 ÷ 24 ≈ 18,700 rows × ~400 B (the 323 B typical delta plus its key columns). Transient, and outside every per-seat-day figure above |
...
`server/vendor/laravel/framework/src/Illuminate/Routing/ResponseFactory.php:131`, laravel/framework
13.26.1). No daemon, no broadcasting package, no protocol library, no websocket-capable proxy module.**
...
Both close by construction rather than by rule.

**Authentication is the route's ordinary middleware.** `EventSource` sends the session cookie
same-origin, so the stream is gated by the same session + MFA stack as the page
...
whole envelope, `t` included, so a client may dispatch on either and the two cannot drift, being one
...
| `fleet.reload` | server → client | a deploy — written by `mezzanine:feed-reload` ([§ 2.1](#21-processes)) before PHP-FPM is reloaded, carrying the release's `feed_version` whether or not it changed, because the wire's version is not the bundle's and the deploy is the one act that knows the bundle moved. ⛔ **Terminal**: the handler ends the stream after delivering it, which is what returns a pinned worker before the reload asks for it | `feed_version`, `reason` |
...
| `feed.close` | server → client | the handler is ending this stream **on its own decision**, and says why: `stalled` — a tick did not complete within [§ 8.5](#85-gaps-reconnect-and-why-state_version-is-not-seq)'s 45 s bound; `session` — [§ 9](#9-read-side-authentication)'s 15 s re-check found the session or its MFA no longer valid. A stream that ends with no `feed.close` ended for a reason the server did not choose (a worker killed, the proxy, the network), and the client treats the two alike except that `session` sends it to sign in rather than to reconnect ([FLOOR.md § 9](FLOOR.md#9-failure-paths-and-their-observables) F3, F7) | `reason`, `at` |
...
index seek returning nothing — which is the design's own 250 ms rather than a number this amendment
mints.
...
line is unbounded by the protocol — and what sits between the handler and the browser buffers by chunk,
...
GET /api/fleet/stream                       -- session + MFA middleware (§ 9); text/event-stream
  ...
  cursor    = MAX(feed_outbox.id)           -- the head at connect: no history, ever (§ 8.5)
  tick_done = now;  auth_done = now
  loop:
     sleep until 250 ms after tick_done
     if now - tick_done > 45 s:             -- the last flush blocked: the client is not draining
         yield feed.close{reason:"stalled"}; count feed_resync_required; return
     rows = SELECT id, t, install_id, message FROM feed_outbox
             WHERE id > cursor AND created_at <= server_now - INTERVAL 2 SECOND   -- § 6.5's lag
             ORDER BY id
     for row in rows:
     ...
     if now - auth_done >= 15 s:
         if not session_and_mfa_still_valid(reader):
...
**The cursor starts at the head** and is set by nothing the client sends, which is what makes the
outbox transient — below. `yield` is `eventStream()`'s write-and-flush; a `yield` to a client that is
not draining blocks in the write, which is why the stall check compares clocks *after* a write returns
and cannot run while one is blocked — [§ 8.5](#85-gaps-reconnect-and-why-state_version-is-not-seq)
prices that.
...
reads. This section is that surface; `bin/deploy.sh` is the far end that runs the checks, and a deploy
that has not run them has not met the design.

**R1 — nothing between PHP-FPM and the browser may buffer the stream.** *Declared:* the handler sends
`X-Accel-Buffering: no` and `Cache-Control: no-cache` on every stream response (the primitive sets both
— verified at `ResponseFactory.php:184-186`), flushes after every message, and never compresses
`text/event-stream`. *What this repository cannot see:* whether the reverse proxy honours that header
for a FastCGI upstream, whether a compression or output filter sits in the path, and what its read
timeout is — the heartbeat's 15 s is what keeps a 60 s default alive, and a timeout below it would end
every stream on a schedule. *The check, run from any host that can reach the deployed origin with a
signed-in session:*

```
curl -sN -b "$COOKIE" https://<host>/api/fleet/stream | head -c 200
```

must print the on-connect `fleet.health` within a second — a buffered stream prints nothing until the
buffer fills, which on a quiet fleet is never — and, left running for **two minutes**, must show
`event: feed.heartbeat` lines arriving fifteen seconds apart with no interruption. *The observable when
...
**R2 — the PHP-FPM pool serving `/api/fleet/stream` must hold one worker per open browser for the
length of a session, and give a dead one back.** *Declared:* a stream is a request that does not end —
...
*The requirement, in those terms:* `pm.max_children` ≥ the number of browsers that will be open at once
plus the REST concurrency the console needs beside them; `request_terminate_timeout` **0** for the pool
that serves the stream (any other value ends every healthy stream on that period, which renders as every
browser reconnecting in lockstep); `process_control_timeout` **non-zero**, so a reload can finish over a
half-open stream whose client vanished without a `FIN` — the one worker the handler cannot return on its
own, because it learns the client is gone only when a write fails, and a half-open TCP connection fails
its writes only after the kernel's retransmission budget. ⭐ **The recommended shape is a dedicated pool
for the stream route**, so that a stream can never starve a snapshot: it costs a second pool definition
and one location rule, and it makes the failure below structurally impossible rather than merely sized
against. *The check:* read the effective pool configuration (`php-fpm -tt`, or the pool file) for the
three values; then open N streams with the R1 command in N shells and, with all N open,
`GET /api/fleet/health` must answer at its ordinary latency and the pool's status page
(`pm.status_path`) must show `listen queue` at 0 and `max children reached` unmoved. *The observable
...
is on **the tick**: when a write returns, the handler compares the clock to the moment its last tick
completed, and a gap over **45 s** ends the stream with `feed.close{reason:"stalled"}` — after which
`feed_resync_required` is counted and the worker is returned. 45 s is the client's own dead-feed
...
bound is not longer. Dropping rows instead of ending the stream is refused for the same reason as
...
this section refuses everywhere else. ⚠ The one worker the bound cannot return is a client that
vanished mid-write without closing: the write blocks until the kernel gives up on the connection, and
that is R2's `process_control_timeout` clause, named there rather than here.
...
residual. An SSE handler is PHP with the store in reach, so **on every heartbeat tick it re-runs the
same check the route's middleware ran at connect — the session is live and its MFA enrolment holds —
and a failed check ends the stream with `feed.close{reason:"session"}`**
([§ 8.3](#83-the-websocket-delta-feed)). The lag between an expiry and its enforcement is therefore at
most one tick, 15 s, during which the stream delivers messages the session was entitled to when the
tick before ran; that window is accepted on the record rather than closed with a per-message check,
because a per-message check is a session read per delta at 5.2 msg/s for a property that moves once a
day. The same clock answers [FLOOR.md § 14](FLOOR.md#14-open-questions-for-the-review-loop) item 5.
...
- **Build:** two open streams; stop **reading** on one — the harness's socket keeps the connection open
  and stops draining it — and drive fleet traffic for longer than
  [§ 8.5](#85-gaps-reconnect-and-why-state_version-is-not-seq)'s 45 s stall bound.
- **GREEN:** the stalled stream is ended by its handler with `feed.close{reason:"stalled"}` as its last
  message — delivered when the harness resumes reading, in order, after every row the handler had
  written — `feed_resync_required` increments **once**, the handler's worker is returned to the pool
...
- **RED — no stall bound:** remove the tick comparison → the worker stays blocked in its write until the
  kernel gives up on the connection, and every such client holds one of `pm.max_children` for minutes;
  with enough of them the snapshot and the health endpoint queue behind the streams, which is R2's
  observable ([§ 8.3](#83-the-websocket-delta-feed)) arriving through a test rather than a host.
- **Second RED — skip rows instead of ending the stream:** on resume, advance the cursor past what the
...
- **GREEN — the stream re-checks:** open a stream under a valid MFA session, then expire the session (or
  clear the user's enrolment) with the stream open → within one 15 s tick the stream's last message is
...
- **RED:** cache the token row for 60 s → a revoked credential keeps reading the fleet for a minute, which
  ...
  becomes a read credential, and every seat's token is now a fleet-wide read grant. **Third RED —
...
- **RED — remove the visibility lag:** drop the `created_at <= server_now - INTERVAL 2 SECOND` term → a
  tick that lands between the two commits advances the cursor past writer 1's lower `id`, and that post
  is **never delivered** to this stream: no counter moves and no client detects it.
- **Second RED — the early insert:** move a writer's outbox `INSERT` from the last statement before
...
| 9 | the stream handler and `feed_outbox` — the on-connect `fleet.health`, deltas in `id` order behind the visibility lag, the feed heartbeat, the stall bound, the session re-check, `mezzanine:feed-reload`; and the retirement of `/broadcasting/auth`, `routes/channels.php` and the broadcast-event wiring the previous transport left in `server/app/Feed` | [AT-D2-7](#at-d2-7-snapshot-then-deltas-has-no-window), [AT-D2-8](#at-d2-8-a-delta-gap-is-detected-and-resynced), [AT-D2-15](#at-d2-15-feed-backpressure-closes-one-connection-and-no-others), [AT-D2-25](#at-d2-25-a-concurrent-writer-cannot-strand-a-message-behind-a-streams-cursor) |
