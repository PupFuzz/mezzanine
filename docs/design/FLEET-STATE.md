...
| **feed reload** | deploy command (`mezzanine:feed-reload`), run by `bin/deploy.sh` **before** the deploy's opcache wait — no deploy reloads PHP-FPM, and the deploy user cannot (the pool's master is root's, `docs/PLAN.md § 5`) | per deploy | write one `fleet.reload` row to `feed_outbox` carrying the release's `feed_version`, **then wait for the streams to go**: a row is invisible to every handler for [§ 6.5](#65-the-fold)'s 2 s visibility lag and is delivered on the tick after that, so the command returns no sooner than **lag + tick + margin (3 s)**, and then polls the stream pool's active-process count (**the pool's `pm.status_path`, which [§ 8.3](#83-the-websocket-delta-feed) R2 requires for exactly this reason**) until it reaches zero **or a 30 s ceiling elapses, whichever comes first — at the ceiling it returns success and logs the residual count**, because a deploy must not block indefinitely on one wedged stream. ⛔ **Nothing ends whatever is left, today — and what should is an owed design decision, not a mechanism this row asserts** ([§ 14](#14-open-questions-for-the-review-loop) item 17). An earlier revision named FPM's own reload escalation as the backstop (R2's `process_control_timeout`), and no deploy reloads FPM any more. The candidate is the deploy sending SIGTERM to the stream-pool workers still serving a stream at the ceiling, found through that pool's `pm.status_path` full listing; that those workers run as the deploy user, so the signal needs no root, and that the pool's root-owned master respawns them, are its premises, and **none of it has been run on a host — UNMEASURED**. ⚠ *Preferably* stood here in an earlier revision, which is not a buildable instruction: one builder writes `sleep 3`, another blocks forever, and the two differ on whether a single frozen client can stall every deploy. Every open stream then delivers it and **ends** ([§ 8.3](#83-the-websocket-delta-feed)), which is what takes it off the previous release's code — opcache revalidation reaches new requests only, and the client's reconnect is a new request — ⚠ a deploy that writes the row and moves on in the next line does so while every stream is still inside the lag window, so it has seen no stream end and cannot tell a stream that took the message from one that missed it | streams keep serving the previous release's code until each stream ends on its own clock — for a healthy client, not before its session expires, `SESSION_LIFETIME` after it connected ([§ 9](#9-read-side-authentication)) — unless a deploy-side backstop ends it (owed; [§ 14](#14-open-questions-for-the-review-loop) item 17), and each client learns of the deploy from the first envelope whose `feed_version` it does not know ([§ 8.1](#81-two-surfaces-two-compatibility-postures)) once it reconnects, rather than from a message that says so — later, and by the failure path instead of the announcement |
...
belong in `bin/deploy.sh` as a gate that refuses on a red. ⭐ **The reader to extend is `fpm_code_reload_ready`
in `bin/deploy.sh`**, which already reads the FPM SAPI's own phpinfo (`php-fpm -i`), the pools running as
the deploy user with their `php_value` / `php_admin_value` / `php_flag` overrides, and the `.user.ini`
files over the app's scripts — for `opcache.*` keys today. Card#9300 widens that reader to R1's and R2's
values rather than writing a second one, and neither becomes a refusal until R1's own measurement rule
is met. R1's **wire** half needs a stream opened
...
is gone rather than relaxed. ⛔ **And BOTH pools
...
decide when the streams have gone before the deploy's opcache wait, and a pool without a status path
answers that poll with nothing; the candidate backstop of [§ 14](#14-open-questions-for-the-review-loop) item 17 would read the same pool's
full listing. Deploy behaviour depends on it, so it is stated here with the pool's other
values rather than as a caveat on an instrument (card#9287, maintainer round).
...
and the deploy user cannot — and a stream that misses it holds the previous release's code until it ends
on its own clock, which no deploy-side backstop bounds yet ([§ 14](#14-open-questions-for-the-review-loop) item 17).
...
17. **⇢ Review / operator — what ends a stream that misses `fleet.reload`, now that no deploy reloads FPM?**
    [§ 2.1](#21-processes)'s `mezzanine:feed-reload` waits for the streams to go up to its ceiling, then
    returns and logs the residual count. The backstop for that residual was FPM's own reload escalation
    ([§ 8.3](#83-the-websocket-delta-feed) R2's `process_control_timeout`), and it went with the reload:
    the deploy user cannot reload a pool whose master is root's, and `bin/deploy.sh` waits out opcache
    revalidation instead (`docs/PLAN.md § 5`). **Today nothing ends such a stream.** It serves the
    previous release's code until it ends on its own clock — for a healthy client, not before its
    session expires ([§ 9](#9-read-side-authentication)), and later still if item 16 is ruled toward a
    longer-lived floor — and its client meets the new release only when it reconnects. **The candidate,
    UNMEASURED:** the deploy sends SIGTERM to the stream-pool workers still serving a stream at the
    ceiling, found through that pool's `pm.status_path` full listing. Its premises — that the listing
    tells a stream-serving worker apart by its request, that those workers run as the deploy user so
    the signal needs no root, and that the pool's root-owned master respawns what it loses — have none
    of them been run on a host, so this document writes the gap at § 2.1's feed-reload row and at R2's
    cost (5) and asserts no mechanism. **Blocks:** nothing yet — neither the stream route nor the
    command exists, so no stream can miss the message; it goes live with
    [Appendix B](#appendix-b--what-an-implementer-builds-from-this) step 9. **Closes it:** the candidate
    run once on a host — a stream held past the ceiling, the signal sent, the worker seen to go and be
    respawned, the client seen to reconnect onto the new release — and then either written into the
    feed-reload row and step 9, or a ruling that a stream bounded by its session is acceptable.
