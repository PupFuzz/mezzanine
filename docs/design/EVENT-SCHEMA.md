...
1. **Supervised start** — the installer registers the flusher with the OS: a `systemd --user` unit on
   ...
   *contract* is here.) ⚠ **That start inherits nothing from the harness's settings**, so the
...
- **DECLARED, to the party who can falsify it.** The contract, stated for whoever provisions a seat
  ...
  runs.** The OS launches it — a `systemd --user` unit, a Scheduled Task at logon — from an
  environment the harness's settings never reach; and because the flusher lock is exclusive and a
  ...
  the multi-agent box this resolution exists for. So **the installer resolves `$COORD_CONFIG` at
  install time, and the supervised start's launch delivers that value to the flusher** — and
  delivers nothing when it is unset there, so a solo box keeps the home-path branch. The route is
  installer `card#7336`'s and is not designed here, and it is not assumed to be an environment the
  start definition carries of its own: a start definition need not carry one.
  ⚠ **A value resolved once goes stale when the coordination config moves.** The launch goes on
  delivering the path as it stood at install, so the same contract requires the installer to
  **rewrite the start definition when that value changes** — `card#7336`'s, like the route. Until it
...
- **Case F — case C's box, the flusher started by the SUPERVISED START with nothing inherited.**
  ...
  the start definition the installer wrote, from an environment carrying no `$COORD_CONFIG` of its own
...
- **Case F, moved — case F's box after the coordination config MOVES.** With case F green, move the
  coordination config, point `$COORD_CONFIG` at its new location, apply the installer's
  rewrite-on-change ([§ 3.1](#31-the-seat-config-file), `card#7336`), and start the flusher again
