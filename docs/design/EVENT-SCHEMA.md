...
1. **Supervised start** — the flusher is registered with a supervisor that runs as the seat's own
   account and needs no root. **On Linux that is the user crontab**: an `@reboot` entry plus a minutely
   entry under `flock -n`. The minutely entry does nothing while its started copy runs, and starts it
   again once it has exited. This is the pattern `bin/supervision.sh` uses for the server's daemons.
   **On Windows** it is a Scheduled Task at logon. A `systemd --user` unit is **not** the Linux start:
   without lingering, the user manager neither starts at boot nor outlives the account's last logout
   (`loginctl(1)`, `enable-linger`), and these hosts run with no lingering and no root (`docs/PLAN.md
   § 5`). The Linux mechanics are `fleet-reporter/INSTALL-LINUX.md`'s (card#9368), which also measures
   how long a restart takes. The Windows installer, card #7336, is won't-do. The *contract* is
   ...
   it, and why the installer rewrites the start when that value changes).
...
- **DECLARED, to the party who can falsify it.** The contract, stated for whoever provisions a seat
  ...
  Windows the route belonged to installer `card#7336`, which is won't-do, so no Windows route exists.
  ...
  **rewrite the start definition when that value changes**. On Linux that means re-running the
  runbook's supervision step with the new path. Until it
...
- **Case F, moved — case F's box after the coordination config MOVES.** With case F green, move the
  ...
  rewrite-on-change ([§ 3.1](#31-the-seat-config-file)), and start the flusher again
