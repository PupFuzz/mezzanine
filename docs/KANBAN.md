...
| File | What it is |
|---|---|
| `.github/workflows/card-token-lint.yml` | PR checks — **two jobs, and only the first is kanban's.** `card-token-lint` REJECTS a card-token spelling the correlators cannot parse; `pr-body-lint` (card#9767) judges the PR BODY against the fleet standard with a vendored copy of upstream's linter and its **verdict on the body is report-only — that step prints findings and exits 0** (the job's other steps judge the checkout and do red), documented by `bin/pr-body-lint.py`'s docstring and by its own block in the workflow, not here. The file keeps its name from the kanban job; its display name does not. **Neither needs a credential — both are live as soon as they land.** |
